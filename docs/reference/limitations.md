...
Phase 08 is in progress. Gates 1–4 implement the exact current-user LaunchAgent lifecycle, level-triggered unattended reconciliation, secure declared-configuration reload, and schema-v17 workload/project restart budgets with deterministic backoff, durable history, crash-loop holds, stable-run reset, priority fairness, and exact manual release. Maintenance windows, expanded unattended rollout/rollback, complete checkpoint/finalizer/GC authority, OSLog, durable watches, metrics, traces, support bundles, and aggregate soak evidence remain unavailable until their owning Phase 08 gates pass.
