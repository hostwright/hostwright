...
final class HostwrightPolicySmoke: XCTestCase {
    ...
    func testTeamPolicyProfileAllowsLocalOptInProfileWithRequiredGates() {
        let profile = TeamPolicyProfile(
            ...
            overrides: [
                TeamPolicyOverride(
                    ...
                    effect: .stricter,
                    justification: "team requires digest-pinned images"
                )
            ]
        )
        ...
        XCTAssertEqual(decisions.map(\.reasonCode), [.teamProfileDeclared])
        XCTAssertEqual(decisions.first?.severity, .allow)
        XCTAssertEqual(decisions.first?.category, .team)
        XCTAssertTrue(decisions.first?.message.contains("local, opt-in, auditable") == true)
    }

    func testTeamPolicyProfileFailsClosedForSilentOrUnsupportedProfile() {
        let profile = TeamPolicyProfile(
            identifier: "",
            version: 2,
            displayName: "Silent Defaults",
            optIn: false,
            requiredGates: []
        )
        ...
        XCTAssertEqual(reasonCodes.filter { $0 == .teamProfileMissingRequiredGate }.count, TeamWorkflowGate.allCases.count)
        XCTAssertTrue(decisions.allSatisfy { $0.severity == .blocker })
    }

    func testTeamPolicyOverrideRequiresApprovedLocalReviewRecord() {
        let missingApproval = TeamPolicyProfile(
            identifier: "dev.hostwright.team.review",
            displayName: "Reviewed Team",
            optIn: true,
            requiredGates: TeamWorkflowGate.allCases,
            overrides: [
                TeamPolicyOverride(
                    kind: .allowPrivilegedPortWarning,
                    effect: .weakensRequiredGate,
                    justification: "temporary local exception",
                    approvalID: "approval-1"
                )
            ]
        )
        let missingDecisions = TeamWorkflowPolicyEvaluator.default.evaluate(missingApproval)
        XCTAssertEqual(missingDecisions.map(\.reasonCode), [.teamOverrideRequiresApproval])
        XCTAssertEqual(missingDecisions.first?.severity, .blocker)

        let approved = TeamPolicyProfile(
            identifier: "dev.hostwright.team.review",
            displayName: "Reviewed Team",
            optIn: true,
            requiredGates: TeamWorkflowGate.allCases,
            ...
            approvals: [
                TeamApprovalRecord(
                    ...
                    reviewer: "maintainer",
                    decision: .approved,
                    scope: "allowPrivilegedPortWarning",
                    recordedAt: "2026-07-09T00:00:00Z"
                )
            ]
        )
        let approvedDecisions = TeamWorkflowPolicyEvaluator.default.evaluate(approved)
        XCTAssertEqual(approvedDecisions.map(\.reasonCode), [.teamApprovalRecorded])
        XCTAssertEqual(approvedDecisions.first?.severity, .warning)
        XCTAssertTrue(approvedDecisions.first?.remediation.contains("do not bypass hard-coded Hostwright safety gates") == true)
    }

    func testTeamPolicyForbiddenOverridesCannotBypassRequiredGates() {
        let profile = TeamPolicyProfile(
            identifier: "dev.hostwright.team.forbidden",
            displayName: "Forbidden Defaults",
            optIn: true,
            requiredGates: TeamWorkflowGate.allCases,
            overrides: [
                TeamPolicyOverride(
                    kind: .bypassPlanConfirmation,
                    effect: .weakensRequiredGate,
                    justification: "unsafe",
                    approvalID: "approval-1"
                ),
                TeamPolicyOverride(
                    kind: .telemetryUpload,
                    effect: .weakensRequiredGate,
                    justification: "unsafe",
                    approvalID: "approval-1"
                )
        ...
        XCTAssertEqual(decisions.map(\.reasonCode), [.teamOverrideForbidden, .teamOverrideForbidden])
        XCTAssertTrue(decisions.allSatisfy { $0.severity == .blocker })
        XCTAssertTrue(decisions.map(\.message).joined(separator: "\n").contains("cannot bypass required Hostwright safety gates"))
    }
