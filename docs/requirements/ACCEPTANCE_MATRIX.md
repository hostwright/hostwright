...
| HW-SAFE-002, HW-SAFE-003, HW-STATE-005 | General cleanup requires ownership records, live observation, non-running lifecycle, dry-run token, and exact confirmation before container delete. Volume deletion remains a separate retain-by-default, policy- and confirmation-bound storage operation. | Automated + manual | CLI cleanup XCTest plus storage reclaim/orphan, fencing, SIGKILL recovery, and unmanaged-preservation suites. |
...
## Phase 08 Gate 14: Correlated Local Traces

| Requirement IDs | Acceptance criteria | Verification type | Verification command or review |
| --- | --- | --- | --- |
| HW-OBS-008, HW-STATE-003 | CLI requests and daemon iterations propagate one canonical trace context through plan, saga, provider, health, rollback, finalizer, cleanup, event, and operation boundaries. A complete failed lifecycle is reconstructible after restart from no more than 64 schema-v1 spans without schema v18. | Unit contract + local integration + migration | Trace contract/state/saga/daemon tests, async context-isolation cells, reopen and v16→v17/future-version evidence, plus a built-CLI lifecycle trace. |
| HW-OBS-008, HW-SAFE-004 | Span names and attributes are fixed and bounded; project/service/resource/path/error/secret/credential/PII values cannot become attributes. Deterministic sampling retains a bounded root summary, failure/cancellation overrides sampling, and sink/exporter failure cannot alter or fabricate control success. | Security assessment + resilience | Limit/cardinality/secret scans, concurrent contexts, degraded sink, failed-lifecycle reconstruction, cancellation, and security review. |
| HW-OBS-008, HW-STATE-009 | Inspect reads existing state only. Export requires one complete trace, its exact recomputed hash, and one new canonical private `0600` file; overwrite/path/symlink/hard-link/cancellation/write failure removes only the proven incomplete file. Trace rows retain independently under `traces`, with no listener or automatic upload. | Live runtime + resilience | CLI inspect/export subprocess, stale-confirmation and filesystem-race cells, exact trace compaction, retention holds, and artifact inventory. |

## Phase 08 Gate 15: Privacy-Safe Support Bundles

| Requirement IDs | Acceptance criteria | Verification type | Verification command or review |
| --- | --- | --- | --- |
| HW-OBS-003, HW-OBS-005, HW-OBS-009 | `diagnostics support` exposes strict schema-v1 status, preview, create, delete, and recover contracts while preserving diagnostics-v1. Preview and create contain only the fixed bounded version, capability, configuration-shape, integrity, redacted OSLog, event, metrics, trace, operation, and evidence projections; raw names, paths, manifest values, payloads, secret stores, credentials, PII, and private keys are excluded. | Unit contract + local integration | Support model/state/CLI suites, diagnostics-v1 regression suite, manifest-shape assertions, adversarial secret/PII scans, and real `dev.hostwright` OSLog collection. |
| HW-OBS-009, HW-SAFE-002, HW-SAFE-004 | Create requires an unchanged preview and one canonical new private `0600` single-link file. Optional macOS CMS encryption uses only bounded stdin/stdout with a non-secret recipient reference and writes no plaintext intermediate. Section, total, encrypted, record, field, and subprocess bounds fail before an unsafe output is acknowledged. | Security assessment + live platform | Private-file and limit tests, disposable real Keychain certificate plus CMS encrypt/decrypt proof, recipient/argv tests, overwrite/symlink/hard-link/path/ACL checks, and focused security review. |
| HW-OBS-009, HW-STATE-009, HW-STATE-012 | Create and delete persist exact private intent before file effects and resume across process loss. Recovery finalizes only a matching device/inode/hash/size identity, compensates a proven no-effect case, or safe-holds ambiguity. Delete additionally requires a retained exact creation receipt and verifies absence without deleting any parent, state, daemon, distribution, or unrelated file. | Migration + resilience + cleanup | Schema-v16 upgrade requirement/future refusal, restart recovery at written/removed checkpoints, changed-identity safe hold, cancellation, exact receipt/link/race deletion matrix, retained support-evidence class, and built-process create/delete/recover cleanup. |
| HW-DOCS-001, HW-DOCS-002, HW-OBS-009 | Reference, operations, security, recovery, local-path, compatibility, limitations, requirement, acceptance, and current-truth docs describe the tested local-only behavior without claiming hosted support, automatic upload, schema v18, Phase 09 transport/auth, aggregate soak, or capability promotion. | Documentation + scope review | Docs build/link/current-truth checks, roadmap governance checks, focused diff and dependency scans. |
...
> **Current Manifest v3 boundary:** complete Compose CPU/memory reservations and limits map to explicit v3 requests and limits. Missing or unrepresentable capacity fails closed with diagnostics and no `hostwright.yaml` text; other deployment semantics remain rejected.
