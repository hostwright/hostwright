...
| State | SQLite schema v22 for standalone/node-local state. It preserves schema-v17 image, storage, network, restart, maintenance, and ownership authority; adds local identity/session state, tamper-evident audit, least-privilege RBAC, admission/profile storage, and immutable plugin lifecycle authority; and enforces one active installed identity per user/team/signing-identifier bucket for atomic code-hash rotation. | Qualified backup/restore/repair and etcd 3.7.x authority for multi-Mac clusters. |
