...
public enum HostwrightCapabilityCatalog {
    ...
    private static let catalog: [HostwrightCapability] = [
        ...
        capability("accelerators.host-native", "Metal, Core ML, and MLX host-native service", .unavailable, 10, 219, "The Phase 10 host-native accelerator contract and service slices remain unavailable pending converged aggregate, live, hardware, and security qualification plus G15; no stable capability is claimed.", [.unitContract, .localIntegration, .securityAssessment, .hardwareBenchmark]),
        capability("api.control-v2", "Persistent local Control API v2", .experimental, 9, 206, "The authenticated bounded unary Unix-socket API enforces tamper-evident authorization, RBAC, deterministic admission, effective-intent reauthorization, and dry-run policy preview; streams, watches, recovery, and complete CLI parity remain incomplete.", [.unitContract, .localIntegration, .securityAssessment]),
        capability("architecture.contracts", "Versioned v0.0.2 public contracts", .experimental, 1, 110, "Manifest v3, Control API v2, Runtime Provider API v2, Storage Provider API v1, Network Provider SPI v1, plugin ABI v1, and state schema v24 are under active qualification.", [.unitContract, .migrationUpgrade]),
        ...
        capability("daemon.reconciliation", "Autonomous level-triggered reconciliation", .unavailable, 8, 194, "Level-triggered reconciliation, restart budgets, maintenance deferral, and health-gated exact-stage rollout are implemented, but autonomous rollback, finalizers, retention, and complete checkpoint recovery remain unqualified.", [.localIntegration, .liveRuntime, .resilienceChaos]),
        ...
        capability("images.supply-chain", "OCI image lifecycle and supply-chain trust", .unavailable, 5, 152, "Strict Apple CLI image lifecycle, registry authentication, provider/platform-bound immutable digest locks, bounded OCI referrer transport, exact keyed/keyless image-signature trust enforcement, exact image-SBOM generation and binding, explainable signed vulnerability-report policy, and exact signed build provenance are implemented; content leases, cache-pressure reclamation, and aggregate Phase 05 evidence remain incomplete.", [.unitContract, .localIntegration, .securityAssessment, .interopConformance]),
        ...
        capability("lifecycle.single-host", "Declarative single-host lifecycle", .experimental, 10, 207, "The historical Phase 04 v2 lifecycle remains qualified for its declared subset. Phase 10 is adding the v3 admission bridge and direct scheduler binding; migrated-fixture convergence, aggregate lifecycle evidence, and full Phase 10 qualification remain pending.", [.unitContract, .localIntegration, .liveRuntime, .migrationUpgrade, .securityAssessment, .resilienceChaos]),
        ...
        capability("manifest.v3", "Manifest v3 contract under qualification", .experimental, 10, 207, "Manifest v3 parsing, deterministic legacy migration, nested resource validation, scheduling policy mapping, and profile-bound admission are under implementation. Migrated-fixture convergence, direct lifecycle admission, aggregate evidence, and Phase 10 qualification remain pending; v3 is not yet a stable support claim.", [.unitContract, .localIntegration, .liveRuntime, .migrationUpgrade]),
        ...
        capability("networking.project", "Project networking, DNS, ingress, policy, and tunnels", .stable, 7, 178, "Hostwright qualifies UUID-owned networks, deterministic project DNS and aliases, dual-stack transport planning, exact port and Unix-socket publication, guarded host access, certificate lifecycle, mTLS identity, fail-closed policy enforcement, authenticated service tunnels, and signed sandboxed provider modules within the Phase 07 support matrix.", [.unitContract, .localIntegration, .liveRuntime, .migrationUpgrade, .securityAssessment, .resilienceChaos, .multiHost, .interopConformance]),
        capability("observability.telemetry", "Events, metrics, traces, logs, and support bundles", .experimental, 8, 194, "The durable local event ledger, retention, redacted diagnostics and OSLog, and bounded cursor/watch recovery are implemented; metrics, traces, support bundles, and leak-qualified soak evidence remain incomplete.", [.localIntegration, .resilienceChaos, .securityAssessment]),
        ...
        capability("registries.authentication", "Private registry authentication", .experimental, 5, 142, "Hostwright provides Keychain-backed login/logout, guarded Docker and OCI credential lookup, bounded Basic and Bearer challenges, exact scopes, token expiry and refresh, TLS-only transport, and same-origin redirect enforcement; Phase 05 live-registry and aggregate evidence remain incomplete.", [.unitContract, .localIntegration, .liveRuntime, .migrationUpgrade, .securityAssessment, .resilienceChaos, .interopConformance]),
        ...
        capability("scheduler.optimization", "Resource scheduler and pressure optimization", .unavailable, 10, 219, "The Phase 10 scheduler contracts, hard filters, fair packing, topology, preemption, and pressure-policy slices are implemented, but optimization remains unavailable pending converged aggregate, live, hardware, and security qualification plus G15.", [.unitContract, .hardwareBenchmark, .resilienceChaos]),
        capability("secrets.keychain", "Production Keychain and secret providers", .experimental, 5, 152, "Production Keychain CRUD and exact workload-scoped Keychain, guarded environment-file, and guarded local-file resolution are implemented; external and plugin references fail closed without explicit registered providers, and aggregate Phase 05 evidence remains incomplete.", [.unitContract, .localIntegration, .securityAssessment]),
        ...
        capability("state.sqlite-v17", "Durable single-host SQLite state schema v17", .stable, 2, 115, "Hostwright-mediated single-Mac state enforces application ownership, private database and sidecar identity, explicit WAL/FULL durability, serialized writers, bounded managed transactions, rollback under cancellation and pressure, fail-closed migration, immutable image and supply-chain evidence, exact content accounting and leases, fenced storage authority, UUID-backed network authority, and durable local restart budgets; same-user direct writers, distributed authority, and arbitrary authoritative salvage are not claimed.", [.unitContract, .localIntegration, .liveRuntime, .migrationUpgrade, .securityAssessment, .resilienceChaos]),
        capability("state.control-identities-v18", "Persistent local control identities and sessions in SQLite schema v18", .experimental, 9, 198, "Schema v18 adds declared peer identities, connection-bound sessions, immediate revocation, request identity, and idempotency foundations while preserving verified v17 backup restoration; it does not expose a listener or grant authorization policy.", [.unitContract, .localIntegration, .migrationUpgrade, .securityAssessment, .resilienceChaos]),
        capability("storage.persistent", "Persistent volumes, snapshots, backup, and restore", .stable, 6, 163, "Storage Provider API v1, the built-in local provider, guarded mounts, named-volume lifecycle, attachment fencing, snapshots, verified backup/restore, quota and pressure accounting, reclaim policy, and orphan quarantine/GC are executable through schema-v17 state and one-shot Control parity. Shared, remote, multi-host, and unmanaged/global storage deletion remain unavailable.", [.unitContract, .localIntegration, .liveRuntime, .migrationUpgrade, .securityAssessment, .resilienceChaos, .interopConformance]),
