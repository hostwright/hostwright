...
Those events and daemon operation records are local forensic inputs for `hostwright events`, `hostwright recovery`, `hostwright diagnostics`, and the explicit local metrics projection. Each new daemon reconciliation operation records one bounded non-negative `durationMilliseconds` value when the clock evidence is valid; legacy/malformed durations remain valid ledger rows but count as unsupported metrics samples. Local cursor pages and bounded long-poll watches read the authoritative SQLite append order, resume through exact event-bound cursors, and report retention gaps without adding a daemon listener. After a durable event transaction commits, a bounded structured mirror is emitted under the `dev.hostwright` OSLog subsystem with the daemon-run correlation identifier. The mirror contains fixed metadata rather than event messages or payloads; it is not authoritative and is never exported or uploaded. See [Durable Events and Local Watches](../reference/events.md), [Local Observability](../reference/observability.md), and [Bounded Local Metrics and SLOs](../reference/metrics.md).
