...
- `hostwrightd --foreground` and the exact managed service mode level-trigger supported drift through the shared fenced lifecycle saga. Phase 08 policy, recovery, authority, retention, OSLog, event-watch, bounded local metrics, and correlated local trace gates are implemented; support bundles, aggregate soak, and final closure remain incomplete.
