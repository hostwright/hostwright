...
| Daemon | Foreground and exact current-user LaunchAgent modes use the same level-triggered lifecycle saga. One-user LaunchAgent lifecycle, secure config reload, unattended reconciliation, durable restart budgets, deterministic local maintenance deferral, and health-gated exact-stage rollout are implemented; later Phase 08 rollback/recovery/observability and soak gates remain incomplete. | Repeat the complete lifecycle on the supported macOS matrix and pass later unattended-policy/soak gates before GA. |
