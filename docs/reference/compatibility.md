...
| Manifest | Strict bounded Yams 6.2.2 decoding, canonical Manifest v2 encoding, complete executable Phase 04 single-host, qualified Phase 07 networking, and Phase 08 restart-budget and maintenance-window fields, plus deterministic legacy-health migration. Unknown or unsupported fields fail before mutation. | Repeat parser, migration, and executable-field qualification at GA. |
| Lifecycle | Confirmed single-host dependency/replica lifecycle, typed probes, bounded interactive operations, rolling/recreate updates, verified rollback, resumable recovery, and unattended restart budgets through one schema-v17 saga. Images must already exist locally and are bound to reviewed descriptor/platform-variant locks. | Repeat current/previous provider and destructive-recovery qualification at GA. |
| Daemon | Foreground and exact current-user LaunchAgent modes use the same level-triggered lifecycle saga. One-user LaunchAgent lifecycle, secure config reload, unattended reconciliation, durable restart budgets, bounded backoff, stable reset, exact manual hold release, and deterministic local maintenance deferral/override are implemented; later Phase 08 rollout/recovery/observability and soak gates remain incomplete. | Repeat the complete lifecycle on the supported macOS matrix and pass later unattended-policy/soak gates before GA. |
...
| State | SQLite schema v17 for standalone/node-local state, including immutable image digest locks, verified OCI referrer graph evidence, image-trust audit, exact image-SBOM records, signed vulnerability-report decisions/exceptions, exact signed build-provenance evidence, provider-scoped content accounting, exact storage volume/snapshot/backup/orphan/quota records, UUID/fence-bound network authority, durable restart budgets/history, and versioned append-only maintenance decisions. | Qualified backup/restore/repair and etcd 3.7.x authority for multi-Mac clusters. |
