...
| --- | --- | --- | --- | --- |
| The product is a Mac-native single-host desired-state control plane above Apple container. | Document 1 | HW-RECON-001, HW-RUNTIME-001, HW-COMPAT-001, HW-COMPAT-002 | Partially implemented | Current repo has CLI, manifest, planning, SQLite state, Apple observation, narrow apply, bounded logs, events, and ownership-gated cleanup. Full lifecycle management is not implemented. |
...
| Reconciliation must load desired state, observe runtime, diff, plan, validate safety gates, persist intent, apply, record events, and observe again. | Document 1 | HW-RECON-001, HW-RECON-002, HW-RECON-003, HW-RECON-004, HW-RECON-005 | Partially implemented | Planning, create, managed start, status observation, and cleanup events exist. Observe-after-apply, daemon reconciliation, and general lifecycle management remain future. |
...
| Secrets must not leak into manifests, logs, events, status, crash reports, fixtures, or docs. | Document 2 | HW-SAFE-004, HW-STATE-004, HW-OBS-003 | Partially implemented | Runtime, state, planning, logs, status/events, and apply tests cover fake secret redaction; redaction remains heuristic. |
