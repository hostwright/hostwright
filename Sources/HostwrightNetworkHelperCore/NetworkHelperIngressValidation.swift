...
enum NetworkHelperIngressValidation {
    ...
    static func validated(
        ...
        guard listeners.count <=
                HostwrightIngressListener.maximumListeners else {
        ...
        for listener in listeners {
            guard HostwrightNetworkIdentity.isValidManifestName(
                  ...
                  listener.exposure.isDefaultLocalhost,
                  (1...65_535).contains(listener.port),
                  !listener.routes.isEmpty,
                  listener.routes.count <=
                    HostwrightIngressListener.maximumRoutes,
                  endpoints.insert(
            ...
            for route in listener.routes {
                guard HostwrightHostAccessPolicy.isValidHostname(
                      ...
                      Set(route.methods).count ==
                        route.methods.count,
                      !route.targetServiceUUIDs.isEmpty,
                      route.targetServiceUUIDs.count <=
                        maximumBackendsPerRoute,
                      route.targetServiceUUIDs.allSatisfy(
                          canonicalUUID
                      ),
                      Set(route.targetServiceUUIDs).count ==
                        route.targetServiceUUIDs.count,
                      (1...65_535).contains(route.targetPort),
                      route.backends.count <=
                        maximumBackendsPerRoute else {
                    throw NetworkHelperError.invalidRequest
                }
                ...
                guard Set(route.methods).isSubset(of: allowedMethods),
                      route.protocolName != .websocket ||
                        route.methods == ["GET"] else {
                    throw NetworkHelperError.invalidRequest
                }
                for method in route.methods {
                    guard routeKeys.insert(
                        [
                            route.hostname,
                            route.pathPrefix,
                ...
                guard route.backends.allSatisfy({
                    targets.contains($0.serviceUUID) &&
                        canonicalUUID($0.serviceUUID) &&
                        canonicalAddress($0.address) &&
                        $0.port == route.targetPort
                }),
                Set(route.backends).count == route.backends.count else {
                    throw NetworkHelperError.invalidRequest
                }
