...
public enum HostwrightCLI {
    ...
    JSON output is supported for capabilities, paths, migrate preview, restart-budget, maintenance, every state, secret, registry, and image subcommand, import-stack, plan, status, every lifecycle command, events, recovery, extension check, doctor, and errors when --json or --output json is present.
