...
| --- | --- | --- | --- | --- |
| The product is a Mac-native single-host desired-state control plane above Apple container. | Document 1 | HW-RECON-001, HW-RUNTIME-001, HW-COMPAT-001, HW-COMPAT-002 | Partially implemented | Current repo has CLI, manifest, planning, SQLite state, read-only Apple observation, and a tightly gated create-only apply path. Full reconciliation and lifecycle management are not implemented. |
...
| Runtime behavior must be isolated behind RuntimeAdapter. | Document 1; Final document | HW-RUNTIME-001, HW-RUNTIME-002, HW-RUNTIME-003, HW-RUNTIME-004 | Implemented for current runtime behavior | Read-only observation and Phase 8B create-only mutation go through `RuntimeAdapter`; later lifecycle actions still require new gates. |
...
| Desired state must be stored durably in SQLite. | Document 1; Final document | HW-STATE-001, HW-STATE-002 | Implemented | Phase 6 added explicit-path SQLite persistence with migrations and repository APIs. |
| Events and operation records must be persisted and survive restart. | Document 1; Document 2; Final document | HW-STATE-003, HW-OBS-001 | Implemented for local ledger records | Phase 6 added event and operation ledgers; Phase 8B records apply started, succeeded, and failed events. |
| Reconciliation must load desired state, observe runtime, diff, plan, validate safety gates, persist intent, apply, record events, and observe again. | Document 1 | HW-RECON-001, HW-RECON-002, HW-RECON-003, HW-RECON-004, HW-RECON-005 | Partially implemented | Phase 7 implements deterministic non-mutating drift planning. Phase 8B adds a single confirmed createMissingService apply path with intent and result records, proven once with a disposable Apple container. Observe-after-apply and general lifecycle management remain future. |
| Apply must support dry-run action plans before mutation. | Document 2 | HW-SAFE-001, HW-CLI-008, HW-RECON-004 | Partially implemented | Phase 8B requires a recomputed plan hash through `--confirm-plan` before the create-only apply gate can execute. |
...
| Secrets must not leak into manifests, logs, events, status, crash reports, fixtures, or docs. | Document 2 | HW-SAFE-004, HW-STATE-004, HW-OBS-003 | Partially implemented | Runtime, state, planning, and Phase 8B apply tests cover fake secret redaction; broader logs/status hardening remains future. |
