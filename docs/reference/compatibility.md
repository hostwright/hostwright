...
| Manifest | Strict bounded Yams 6.2.2 decoding, canonical Manifest v2 encoding, complete executable Phase 04 single-host fields, and deterministic legacy-health migration. Later-phase fields fail before mutation. | Repeat parser, migration, and executable-field qualification at GA. |
| Lifecycle | Confirmed single-host dependency/replica lifecycle, typed probes, bounded interactive operations, rolling/recreate updates, verified rollback, and resumable recovery through one schema-v15 saga. Images must already exist locally and are bound to reviewed descriptor/platform-variant locks. | Repeat current/previous provider and destructive-recovery qualification at GA. |
...
| State | SQLite schema v15 for standalone/node-local state, including immutable image digest locks, verified OCI referrer graph evidence, image-trust audit, exact image-SBOM records, signed vulnerability-report decisions/exceptions, exact signed build-provenance evidence, provider-scoped content accounting, and exact storage volume/snapshot/backup/orphan/quota records. | Qualified backup/restore/repair and etcd 3.7.x authority for multi-Mac clusters. |
