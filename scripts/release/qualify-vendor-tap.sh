...
readonly baseline_version="0.0.2-dev.7"
readonly candidate_version="0.0.2-dev.8"
...
qualify_package_lifecycle() {
  ...
  verify_package_state install-dev7 "$baseline_version" "$HOSTWRIGHT_BASELINE_RELEASE_COMMIT" 1 0.0.2.7 0.0.2.7 0.0.2.7
  sudo -n /usr/sbin/installer -pkg "$baseline_package" -target /
  verify_package_state repair-dev7 "$baseline_version" "$HOSTWRIGHT_BASELINE_RELEASE_COMMIT" 2 0.0.2.7 0.0.2.7 0.0.2.7
  sudo -n /usr/sbin/installer -pkg "$candidate_package" -target /
  verify_package_state upgrade-dev8 "$candidate_version" "$HOSTWRIGHT_CANDIDATE_RELEASE_COMMIT" 3 0.0.2.8 0.0.2.8 0.0.2.8

  distribution="$package_prefix/bin/hostwright-dist"
  sudo -n "$distribution" rollback --prefix "$package_prefix" --output json \
    > "$HOSTWRIGHT_QUALIFICATION_ROOT/rollback-dev7-package-result.json"
  verify_package_state rollback-dev7 "$baseline_version" "$HOSTWRIGHT_BASELINE_RELEASE_COMMIT" 4 0.0.2.7 0.0.2.8 0.0.2.8
  sudo -n /usr/sbin/installer -pkg "$baseline_package" -target /
  verify_package_state repair-after-rollback-dev7 "$baseline_version" "$HOSTWRIGHT_BASELINE_RELEASE_COMMIT" 5 0.0.2.7 0.0.2.7 0.0.2.7
  sudo -n /usr/sbin/installer -pkg "$candidate_package" -target /
  verify_package_state upgrade-again-dev8 "$candidate_version" "$HOSTWRIGHT_CANDIDATE_RELEASE_COMMIT" 6 0.0.2.8 0.0.2.8 0.0.2.8

  before="$(package_snapshot_digest "$work")"
  downgrade_output="$(sudo -n /usr/sbin/installer -pkg "$baseline_package" -target / 2>&1)" \
    || downgrade_status=$?
  [[ "$downgrade_status" -eq 1 ]] || die "The dev.7 package downgrade was not refused." 70
  [[ "$downgrade_output" == *"$package_downgrade_refusal"* ]] \
    || die "The dev.7 package failure did not prove Hostwright's semantic downgrade refusal." 70
...
resume() {
  ...
  brew services restart "$formula_reference"
  wait_for_service
  record "dev7-to-dev8-brew-upgrade-and-service-restart-passed"
