...
| Lifecycle | Confirmed single-host dependency/replica lifecycle, typed probes, bounded interactive operations, rolling/recreate updates, verified rollback, and resumable recovery through one schema-v8 saga. Images must already exist locally and are bound to reviewed descriptor/platform-variant locks. | Repeat current/previous provider and destructive-recovery qualification at GA. |
...
| State | SQLite schema v14 for standalone/node-local state, including immutable image digest locks, verified OCI referrer graph evidence, image-trust audit, exact image-SBOM records, signed vulnerability-report decisions/exceptions, exact signed build-provenance evidence, and provider-scoped content accounting, pins, and fenced leases. | Qualified backup/restore/repair and etcd 3.7.x authority for multi-Mac clusters. |
