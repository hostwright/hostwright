...
- Experimental Phase 10 scheduler/admission contracts for canonical requests, limits, hard placement filters, profile gates, and durable reservation bindings. Manifest v3/lifecycle admission and aggregate qualification remain pending; no stable scheduler or accelerator capability claim is made.
...
The stack-file importer is also not a general YAML or Compose parser. It assesses only the reviewed import subset and fails closed for unsupported networking, discovery, build, deploy, secret, config, named-volume, shell-healthcheck, lifecycle, cloud, and tunnel semantics. It never infers or translates v3 resource requests and limits: a missing manual resource mapping returns diagnostics and no manifest text. Successful import output is text for operator review; it does not write manifests, observe runtime, pull images, inspect registries, or run compatibility shims.
...
Phase 10 scheduler admission is experimental and evidence-gated. `HostwrightScheduler` produces pure deterministic decisions, `HostwrightState` owns durable capacity and reservation authority, and the lifecycle bridge validates requests, limits, policy, and runtime enforceability before mutation. Current-source G3-G8 qualification is sealed with zero safety mismatches and 382 retained intentional optimization-gap fixtures, but lifecycle/Control 2.2 integration, remaining G13-G15 aggregate evidence, remote placement, and accelerator execution remain unavailable.
...
Hostwright persists adapter-shaped observed state and can consume runtime-shaped observed state in memory for planning. Apply, status, logs, events, diagnostics, cleanup, and both `hostwrightd` modes use the same secure local-path resolver. The managed daemon may reconcile only supported owned drift through the existing lifecycle saga; it still provides no broad cleanup, external telemetry, Phase 09 listener, multi-host authority, or unsupported production claim.
