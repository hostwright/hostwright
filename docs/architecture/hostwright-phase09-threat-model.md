...
| P09-T02 | PID reuse or stale session replay | Identity confusion | helper live-process validation | bind PID version, audit token, daemon generation, socket identity, nonce; revoke active sessions (G2) | Kernel identity collection failure denies service. |
