...
public enum HostwrightCapabilityCatalog {
    ...
    private static let catalog: [HostwrightCapability] = [
        ...
        capability("architecture.contracts", "Versioned v0.0.2 public contracts", .experimental, 1, 110, "Manifest v2, Control API v2, Runtime Provider API v2, plugin ABI v1, and state schema v14 are under active qualification.", [.unitContract, .migrationUpgrade]),
        ...
        capability("lifecycle.single-host", "Complete declarative single-host lifecycle", .stable, 4, 140, "Manifest v2 workloads execute through a durable schema-v8 operation DAG with dependency reconciliation, lifecycle and bounded interactive commands, typed probes, rolling or recreate updates, verified rollback, resumable recovery, and exact ownership-scoped cleanup.", [.unitContract, .localIntegration, .liveRuntime, .migrationUpgrade, .securityAssessment, .resilienceChaos]),
        ...
        capability("state.sqlite-v14", "Durable single-host SQLite state schema v14", .stable, 2, 115, "Hostwright-mediated single-Mac state enforces application ownership, private database and sidecar identity, explicit WAL/FULL durability, serialized writers, bounded managed transactions, rollback under cancellation and pressure, fail-closed migration, immutable image digest, trust, SBOM, vulnerability, provenance, and fenced OCI referrer evidence, plus exact content accounting and leases; same-user direct writers, distributed authority, and arbitrary authoritative salvage are not claimed.", [.unitContract, .localIntegration, .liveRuntime, .migrationUpgrade, .securityAssessment, .resilienceChaos]),
