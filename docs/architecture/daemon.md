...
`hostwrightd` supports the existing foreground development loop and a managed per-user LaunchAgent mode. The LaunchAgent lifecycle is controlled only through `hostwright daemon`; `hostwright-dist` remains the executable-distribution authority. Neither mode is a privileged helper, and this Gate 1 lifecycle does not yet authorize unattended runtime mutation.
...
Both daemon modes use the read-only local runtime adapter and do not call `RuntimeAdapter.execute` at Gate 1. They may observe, run bounded in-process loopback health checks, persist restart policy state, and plan actions, but they do not create, start, stop, restart, delete, repair, clean up, or otherwise mutate runtime resources.
