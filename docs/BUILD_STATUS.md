...
- Every ordinary SQLite store connection holds a shared identity-derived state-access fence. Restore, repair, and recovery hold it exclusively. Existing fences are validated before any permission mutation. A pending strict `0600` journal blocks reads, writes, migrations, and missing-database creation until exact recovery.
