...
Schema version 24 is the latest supported state schema. A database migrated by a newer Hostwright release fails closed with an incompatible-schema error. Hostwright does not downgrade state databases or silently convert provider ownership. Migration v16→v17 remains the additive, backup-compatible Phase 08 migration. Phase 09 v17→v18 adds peer identities, control sessions, revocations, request/idempotency foundations, and bounded canonical response persistence for exact mutation replay; v18→v19 adds audit segments, records, key metadata, and retention anchors; v19→v20 adds RBAC roles, bindings, delegations, admission policies and exceptions, and workload-profile storage; v20→v21 adds immutable plugin packages, provenance, grants, activations, revocations, quarantine, and rollback state; v21→v22 enforces one active installed identity per user/team/signing-identifier bucket so a code-hash rotation replaces the same subject atomically and revokes old-hash sessions; v22→v23 adds scheduler capacity, fencing, decisions, reservations, fairness, disruption, preemption, and host-pressure tables; v23→v24 adds dedicated append-only accelerator authority journal/current tables for inventory, claims, reservations, grants, executions, results, usage, provenance, cancellation, and revocation. Accelerator authority is not stored solely in the observability event ledger. Admission and workload-profile repositories verify canonical JSON and SHA-256 content identities on every read, and state integrity repeats those semantic checks. Profile integrity also rejects missing parents, cycles, and chains beyond the frozen depth; generation updates are transactional. Every upgrade uses the verified pre-migration snapshot path; rollback restores that snapshot instead of down-migrating. Metrics, traces, and support bundles continue to reuse bounded projections and event authority. Support-bundle creation/deletion receipts use the independent `supportEvidence` retention class, while a private adjacent file-effect journal carries the exact output path and device/inode identity during recovery; bundle content and paths never enter SQLite.

Gate 8 adds no table or migration. Existing event rows project into schema-v1 stream records at read time. An opaque cursor binds the exact event identifier and SHA-256 of the stored redacted row, then resolves the row's current SQLite append position during each shared read lock. Retained cursors therefore survive authoritative `VACUUM`; deleted anchors become explicit retention gaps and modified anchors fail integrity. Cursor/watch reads do not create, migrate, repair, or compact state. See [Durable Events and Local Watches](../reference/events.md).
...
- `healthy`: SQLite structure, foreign keys, schema v24 ledger/checksums, required tables, indexes, and security triggers, resource/fencing UUIDs, authoritative enums/JSON contracts, and reconstructible projections all pass;
...
Restore has a mandatory two-step contract with the daemon stopped. Restore, repair, maintenance recovery, and compaction use the signed bootstrap companion instead of recording requests in the database they inspect or replace. A shared daemon lifetime lease and exclusive offline recovery lease use the same private per-database lock, including when a daemon uses a custom instance-lock path. Daemon startup and offline recovery exclude one another. Existing state-access fences and stale-confirmation checks still apply.
