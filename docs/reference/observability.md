...
Phase 08 Gates 11–14 provide structured OSLog, durable local event cursors and bounded long-poll watches, a fixed-cardinality schema-v1 metrics/SLO projection, and bounded correlated local traces. Metrics and traces read or append only through retained schema-v17 authority, expose confirmation-bound private file exports, and add no listener or automatic upload; see [Bounded Local Metrics and SLOs](metrics.md) and [Correlated Local Traces](traces.md). Privacy-safe support bundles and aggregate soak qualification remain owned by Gates 15 and 16. The `observability.telemetry` capability therefore remains `experimental` until the aggregate Gate 16 evidence passes.
