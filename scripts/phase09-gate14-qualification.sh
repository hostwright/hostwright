...
readonly schema='hostwright.phase09.gate14.qualification.manifest.v1'
readonly gate=14
...
contract() {
  /bin/cat <<'EOF'
Phase 09 Gate 14 qualification harness contract v1
Gate 14 — 87.50% — aggregate security and resilience.
diagnose is non-qualifying and emits canonical JSON with claim:"none"; it never creates or reuses an evidence root.
Formal prepare/run require one passed, CMS-verified Gate 13 root with matching source and dependency digests.
The frozen product baseline is exactly 500 executions across six serial U/I/L/M/S/R cells; Gate 13/14 harness tests
are excluded from that baseline. Missing or extra exact testcases, zero-test filters, duplicate names, and any count drift fail closed.
Every cell revalidates source, configuration, toolchain, matrix, and Gate 13 evidence before and after execution.
A failed root and both locks are preserved; a partial root is never rerun or repaired. Passed evidence is reused only
after exact state, checksum-manifest, and CMS verification. No public action or broad cleanup is performed.
EOF
}
...
validate_root() {
  ...
  local canonical root_parent
  [[ -d "$parent" && ! -L "$parent" ]] || die 'qualification parent must be an existing canonical directory.' 66
  canonical="$(/bin/realpath "$parent")"
  if testing; then
    [[ "$canonical" =~ ^/private/var/folders/.*/T/hostwright-phase09-gate14-harness-[a-f0-9-]+$ || "$canonical" =~ ^/var/folders/.*/T/hostwright-phase09-gate14-harness-[a-f0-9-]+$ ]] || die 'test parent must be an isolated Gate 14 harness directory.' 66
  else
    [[ "$canonical" == "$live_parent" ]] || die 'Gate 14 evidence must use the fixed qualification parent.' 66
  fi
  ...
  [[ "${root##*/}" =~ ^phase09-gate14-[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$ ]] || die 'Gate 14 evidence root has an invalid lowercase UUID name.' 66
  [[ "$(/usr/bin/stat -f '%u' "$root")" == "$(/usr/bin/id -u)" && "$(/usr/bin/stat -f '%Lp' "$root")" == 700 ]] || die 'Gate 14 evidence root must be current-user-owned mode 0700.' 66
}

empty_root() {
  [[ -z "$(/usr/bin/find "$root" -mindepth 1 -maxdepth 1 -print -quit)" ]] || die 'Gate 14 evidence root must be empty.' 73
}

clean_source() {
  "$git_path" diff --quiet || die 'Gate 14 source must be clean and committed.' 73
  "$git_path" diff --cached --quiet || die 'Gate 14 source must be clean and committed.' 73
  local dirty
  dirty="$("$git_path" status --porcelain=v1 --untracked-files=all | "$awk_path" '$2 !~ /^tmp(\/|$)/ { print }')"
  [[ -z "$dirty" ]] || die 'Gate 14 source must be clean and committed.' 73
}
...
source_digest() {
  {
    ...
    } | LC_ALL=C /usr/bin/sort -z -u | "$python_path" -c 'import hashlib, os, sys
...
validate_formal_tool() {
  local path="$1"
  [[ "$path" == /* && -x "$path" && ! -L "$path" && "$(/bin/realpath "$path")" == "$path" ]] || die "Gate 14 formal tool is not an exact canonical executable: $path" 69
}
...
matrix_validate() {
  [[ -f "$matrix_path" && ! -L "$matrix_path" ]] || die 'Gate 14 aggregate matrix is unavailable or symlinked.' 69
  [[ "$(/usr/bin/jq -r '.schema' "$matrix_path")" == 'hostwright.phase09.gate14.aggregate-matrix.v1' ]] || die 'Gate 14 matrix schema mismatch.' 69
  [[ "$(/usr/bin/jq -r '.gate' "$matrix_path")" == '14' && "$(/usr/bin/jq -r '.version' "$matrix_path")" == 'v1' ]] || die 'Gate 14 matrix gate or version mismatch.' 69
  [[ "$(sha "$matrix_path")" == "$frozen_matrix_digest" ]] || die 'Gate 14 aggregate matrix is not the frozen v1 contract.' 69
  [[ "$(/usr/bin/jq -r '.productTestCount' "$matrix_path")" == "$product_test_count" ]] || die 'Gate 14 product-test baseline is not exactly 500.' 69
  [[ "$(/usr/bin/jq -r '.cells | length' "$matrix_path")" == 6 && "$(/usr/bin/jq -r '[.cells[].cell] | sort | join(",")' "$matrix_path")" == '1,2,3,4,5,6' ]] || die 'Gate 14 matrix must define six ordered cells.' 69
  [[ "$(/usr/bin/jq -c '[.cells[].expectedTests]' "$matrix_path")" == '[72,84,78,64,126,76]' ]] || die 'Gate 14 per-cell execution counts drifted.' 69
  /usr/bin/jq -e '.cellOrder == [1,2,3,4,5,6] and .structuredResult.format == "xunit-v1" and .structuredResult.selectorBinding == "testcase-classname-and-name" and .structuredResult.requireEveryTestcase == true and .structuredResult.exactMultiset == true and .structuredResult.uniqueTestcaseIdentifiers == true and .structuredResult.consoleTextIsNonEvidence == true and [.cells[].evidenceClass] == ["U","I","L","M","S","R"]' "$matrix_path" >/dev/null || die 'Gate 14 cell order, evidence classes, or structured result contract drifted.' 69
  [[ "$(/usr/bin/jq -r '[.cells[].expectedTests] | add' "$matrix_path")" == "$product_test_count" ]] || die 'Gate 14 cell counts do not sum to exactly 500.' 69
  [[ "$(/usr/bin/jq -r '[.cells[].expectedTests] | any(. == 0)' "$matrix_path")" == 'false' ]] || die 'Gate 14 zero-test cell is forbidden.' 69
  [[ "$(/usr/bin/jq -r '[.cells[].testcases[]] | length' "$matrix_path")" == "$product_test_count" ]] || die 'Gate 14 matrix must enumerate exactly 500 testcases.' 69
  [[ "$(/usr/bin/jq -r '[.cells[].testcases[].id] | unique | length' "$matrix_path")" == "$product_test_count" ]] || die 'Gate 14 testcase identifiers must be unique.' 69
  /usr/bin/jq -e 'all(.cells[]; (.evidenceClass | test("^[UILMSR]$")) and ((.testcases | length) == .expectedTests) and all(.testcases[]; (.id | test("^g14-[0-9]{4}$")) and (.classname | test("^[A-Za-z0-9_]+$")) and (.name | test("^test[A-Za-z0-9_]+$"))))' "$matrix_path" >/dev/null || die 'Gate 14 matrix contains an invalid or incomplete testcase.' 69
  /usr/bin/jq -e 'all(.cells[].testcases[]; (.classname | contains("QualificationHarness") or contains("Phase09")) | not)' "$matrix_path" >/dev/null || die 'Gate 14 baseline may not include qualification harness tests.' 69
  /usr/bin/jq -e '([.cells[].testcases[] | "\(.classname)/\(.name)"] | length) == ([.cells[].testcases[] | "\(.classname)/\(.name)"] | unique | length)' "$matrix_path" >/dev/null || die 'Gate 14 matrix contains duplicate testcase names.' 69
  [[ "$(/usr/bin/jq -r '.harnessTestsExcludedFromBaseline | length' "$matrix_path")" == 3 ]] || die 'Gate 14 harness exclusion list drifted.' 69
}

matrix_digest() { sha "$matrix_path"; }

gate13_digest_inventory() {
  printf '%s\n' manifest-v1.json dependency-evidence-v1.json phase08-completion-receipt-v1.json phase08-completion-receipt-v1.sha256 phase08-completion-receipt-v1.cms state-v1.tsv ownership-v1.tsv toolchain-v1.txt gate-active-run-v1-info.tsv
  for cell in 1 2 3 4 5 6; do printf 'cell-%02d.stdout.log\ncell-%02d.stderr.log\ncell-%02d.xunit.xml\n' "$cell" "$cell" "$cell"; done
}

gate13_checksum_inventory() {
  for cell in 1 2 3 4 5 6; do printf 'cell-%02d.stderr.log\ncell-%02d.stdout.log\ncell-%02d.xunit.xml\n' "$cell" "$cell" "$cell"; done
...
validate_gate13_structured_result() {
  local result="$1" cell="$2" selectors expected
  selectors="$(/usr/bin/jq -c --argjson cell "$cell" '[.tests[]|select(.cell==$cell)|.selector]' "$gate13_matrix_path")"
  expected="$(/usr/bin/jq -r --argjson cell "$cell" '[.tests[]|select(.cell==$cell)]|length' "$gate13_matrix_path")"
...
root = ET.fromstring(raw); wanted = set(json.loads(selectors)); seen = set(); cases = list(root.iter('testcase'))
for case in cases:
    if list(case): raise SystemExit(1)
    ident = f'{case.attrib.get("classname", "").rsplit(".", 1)[-1]}/{case.attrib.get("name", "")}'
    if ident in seen or ident not in wanted: raise SystemExit(1)
    seen.add(ident)
...
validate_gate13_inventory() {
  local dep="$1" expected actual file cell expected_checksum
  expected="$( { gate13_digest_inventory; printf '%s\n' evidence-v1.sha256 evidence-v1.cms; } | LC_ALL=C /usr/bin/sort)"
  actual="$(/usr/bin/find "$dep" -mindepth 1 -maxdepth 1 -exec /usr/bin/basename {} \; | LC_ALL=C /usr/bin/sort)"
  [[ "$expected" == "$actual" ]] || return 1
  while IFS= read -r file; do [[ -z "$file" ]] || private_file_0600 "$dep/$file" || return 1; done <<<"$actual"
  expected_checksum="$(while IFS= read -r file; do printf '%s  %s\n' "$(sha "$dep/$file")" "$file"; done < <(gate13_checksum_inventory))" || return 1
  if ! printf '%s\n' "$expected_checksum" | /usr/bin/cmp -s "$dep/evidence-v1.sha256" -; then return 1; fi
  for cell in 1 2 3 4 5 6; do validate_gate13_structured_result "$dep/cell-$(printf '%02d' "$cell").xunit.xml" "$cell" || return 1; done
}
...
collect() {
  ...
  dependency_file="${HOSTWRIGHT_PHASE09_GATE14_DEPENDENCY_EVIDENCE:-}"
...
make_private_temp() {
  local path
  path="$(secure_fs create "$root" ".gate14-${1}")" || die 'Gate 14 could not create an exclusive root-local temporary.' 73
  private_file_0600 "$path" || die 'Gate 14 temporary failed private identity validation.' 73
  printf '%s\n' "$path"
}

make_private_directory() {
  local path
  path="$(secure_fs create-dir "$root" ".gate-external-$gate-$1")" || die 'Gate 14 could not create an unpredictable private external-output directory.' 73
  [[ -d "$path" && ! -L "$path" && "$(/bin/realpath "$path")" == "$path" && "$(/usr/bin/stat -f '%u' "$path")" == "$(/usr/bin/id -u)" && "$(/usr/bin/stat -f '%Lp' "$path")" == 700 ]] || die 'Gate 14 external-output directory failed private identity validation.' 73
  printf '%s\n' "$path"
}
...
validate_checksum_manifest() {
  ...
  [[ "$count" -gt 0 && "$(/usr/bin/awk '{print $2}' "$checksum" | /usr/bin/sort | /usr/bin/uniq -d | /usr/bin/wc -l | /usr/bin/tr -d ' ')" == 0 ]]
}
...
validate_gate13_receipt_file() {
  local receipt="$1" checksum="$2" cms="$3" receipt_identity checksum_identity cms_identity
  [[ "${receipt##*/}" == phase08-completion-receipt-v1.json ]] || return 1
  private_file_0600 "$receipt" && private_file_0600 "$checksum" && private_file_0600 "$cms" || return 1
  receipt_identity="$(file_identity "$receipt")"; checksum_identity="$(file_identity "$checksum")"; cms_identity="$(file_identity "$cms")"
  /usr/bin/jq -e --arg commit "$source_commit" --arg cert "$signing_certificate_sha256" --arg fingerprint "$signing_fingerprint" --arg team "$signing_team_id" --arg identity "$signing_identity" \
    'type=="object" and (keys|sort)==["cmsVerified","finalEvidenceCMSCertificateSHA256","finalEvidenceCMSDigest","finalEvidenceCMSFingerprint","finalEvidenceCMSIdentity","finalEvidenceCMSTeamID","finalEvidenceDigest","schema","sourceCommit","status"] and .schema=="hostwright.phase09.phase08-completion-receipt.v1" and .status=="passed" and .sourceCommit==$commit and (.sourceCommit|test("^[0-9a-f]{40}$")) and (.finalEvidenceDigest|test("^[a-f0-9]{64}$")) and (.finalEvidenceCMSDigest|test("^[a-f0-9]{64}$")) and .finalEvidenceCMSCertificateSHA256==($cert|ascii_downcase) and .finalEvidenceCMSFingerprint==$fingerprint and .finalEvidenceCMSTeamID==$team and .finalEvidenceCMSIdentity==$identity and .cmsVerified==true and (.finalEvidenceCMSCertificateSHA256|test("^[a-f0-9]{64}$"))' "$receipt" >/dev/null || return 1
  [[ "$(/usr/bin/wc -l < "$checksum" | /usr/bin/tr -d ' ')" == 1 && "$(/usr/bin/sed -n '1p' "$checksum")" == "$(sha "$receipt")  ${receipt##*/}" ]] || return 1
  verify_cms_payload "$cms" "$checksum" gate13-receipt || return 1
  private_file_0600 "$receipt" && private_file_0600 "$checksum" && private_file_0600 "$cms" || return 1
  [[ "$(file_identity "$receipt")" == "$receipt_identity" && "$(file_identity "$checksum")" == "$checksum_identity" && "$(file_identity "$cms")" == "$cms_identity" ]]
}
...
validate_dependency_record() {
  local record="$1" root_basename manifest_digest checksum_digest cms_digest dep_root record_source record_source_digest record_config_digest record_toolchain_digest record_matrix_digest record_dependency_digest record_dependency_canonical_digest manifest_gate manifest_state manifest_source manifest_matrix manifest_source_digest manifest_config_digest manifest_toolchain_digest manifest_dependency_digest manifest_dependency_canonical_digest manifest_receipt_digest manifest_receipt_checksum_digest manifest_receipt_cms_digest actual_dependency_digest actual_dependency_canonical_digest
  /usr/bin/jq -e --arg cert "$(lowercase "$signing_certificate_sha256")" --arg fingerprint "$signing_fingerprint" --arg team "$signing_team_id" --arg identity "$signing_identity" \
    'type=="object" and (keys|sort)==["checksumManifestDigest","cmsCertificateSHA256","cmsDigest","cmsFingerprint","cmsIdentity","cmsTeamID","configDigest","dependencyEvidenceCanonicalDigest","dependencyEvidenceDigest","gate","manifestDigest","matrixDigest","rootBasename","sourceCommit","sourceDigest","status","toolchainDigest"] and .gate==13 and .status=="passed" and (.rootBasename|test("^phase09-gate13-[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$")) and (.sourceCommit|test("^[0-9a-f]{40}$")) and (.sourceDigest|test("^[a-f0-9]{64}$")) and (.configDigest|test("^[a-f0-9]{64}$")) and (.toolchainDigest|test("^[a-f0-9]{64}$")) and (.matrixDigest|test("^[a-f0-9]{64}$")) and (.dependencyEvidenceDigest|test("^[a-f0-9]{64}$")) and (.dependencyEvidenceCanonicalDigest|test("^[a-f0-9]{64}$")) and (.manifestDigest|test("^[a-f0-9]{64}$")) and (.checksumManifestDigest|test("^[a-f0-9]{64}$")) and (.cmsDigest|test("^[a-f0-9]{64}$")) and .cmsCertificateSHA256==($cert|ascii_downcase) and .cmsFingerprint==$fingerprint and .cmsTeamID==$team and .cmsIdentity==$identity' <<<"$record" >/dev/null || die 'Gate 14 Gate 13 dependency record or signer binding is invalid.' 69
  root_basename="$(/usr/bin/jq -r '.rootBasename' <<<"$record")"
  [[ "$root_basename" =~ ^phase09-gate13-[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$ ]] || die 'Gate 14 Gate 13 root basename is invalid.' 69
  [[ -z "$(/usr/bin/jq -r '(.path // .rootPath // empty)' <<<"$record")" ]] || die 'Gate 14 dependency records may not substitute a path for a bound root basename.' 69
  record_source="$(/usr/bin/jq -r '.sourceCommit' <<<"$record")"
  [[ "$record_source" == "$source_commit" && "$record_source" =~ ^[0-9a-f]{40}$ ]] || die 'Gate 14 Gate 13 evidence is not bound to the current source commit.' 69
  ...
  [[ -d "$dep_root" && ! -L "$dep_root" && "$(/bin/realpath "$dep_root")" == "$dep_root" && "$(/bin/realpath "$(path_parent "$dep_root")")" == "$evidence_parent" ]] || die 'Gate 14 Gate 13 root is not a direct canonical child of the evidence parent.' 69
  [[ "$(/usr/bin/stat -f '%u' "$dep_root")" == "$(/usr/bin/id -u)" && "$(/usr/bin/stat -f '%Lp' "$dep_root")" == 700 ]] || die 'Gate 14 Gate 13 root ownership is invalid.' 69
  private_file_0600 "$dep_root/manifest-v1.json" || die 'Gate 14 Gate 13 manifest is unsafe or not private.' 69
  private_file_0600 "$dep_root/evidence-v1.sha256" || die 'Gate 14 Gate 13 checksum manifest is unsafe or not private.' 69
  private_file_0600 "$dep_root/evidence-v1.cms" || die 'Gate 14 Gate 13 CMS is unsafe or not private.' 69
  private_file_0600 "$dep_root/dependency-evidence-v1.json" || die 'Gate 14 Gate 13 dependency evidence is unsafe or not private.' 69
  [[ "$(sha "$dep_root/manifest-v1.json")" == "$manifest_digest" && "$(sha "$dep_root/evidence-v1.sha256")" == "$checksum_digest" && "$(sha "$dep_root/evidence-v1.cms")" == "$cms_digest" ]] || die 'Gate 14 Gate 13 evidence digest changed.' 69
  manifest_gate="$(/usr/bin/jq -r '.gate' "$dep_root/manifest-v1.json")"; manifest_state="$(/usr/bin/jq -r '.status' "$dep_root/manifest-v1.json")"; manifest_source="$(/usr/bin/jq -r '.sourceCommit' "$dep_root/manifest-v1.json")"; manifest_matrix="$(/usr/bin/jq -r '.matrixDigest' "$dep_root/manifest-v1.json")"; manifest_source_digest="$(/usr/bin/jq -r '.sourceDigest' "$dep_root/manifest-v1.json")"; manifest_config_digest="$(/usr/bin/jq -r '.configDigest' "$dep_root/manifest-v1.json")"; manifest_toolchain_digest="$(/usr/bin/jq -r '.toolchainDigest' "$dep_root/manifest-v1.json")"; manifest_dependency_digest="$(/usr/bin/jq -r '.dependencyEvidenceDigest' "$dep_root/manifest-v1.json")"; manifest_dependency_canonical_digest="$(/usr/bin/jq -r '.dependencyEvidenceCanonicalDigest' "$dep_root/manifest-v1.json")"; manifest_receipt_digest="$(/usr/bin/jq -r '.phase08CompletionReceiptDigest' "$dep_root/manifest-v1.json")"; manifest_receipt_checksum_digest="$(/usr/bin/jq -r '.phase08CompletionReceiptChecksumDigest' "$dep_root/manifest-v1.json")"; manifest_receipt_cms_digest="$(/usr/bin/jq -r '.phase08CompletionReceiptCMSDigest' "$dep_root/manifest-v1.json")"
  [[ "$manifest_gate" == 13 && "$manifest_state" == 'passed' && "$manifest_source" == "$source_commit" && "$manifest_source" =~ ^[0-9a-f]{40}$ && "$manifest_matrix" == "$record_matrix_digest" && "$manifest_matrix" == "$(sha "$gate13_matrix_path")" && "$manifest_source_digest" == "$record_source_digest" && "$manifest_config_digest" == "$record_config_digest" && "$manifest_toolchain_digest" == "$record_toolchain_digest" && "$manifest_dependency_digest" == "$record_dependency_digest" && "$manifest_dependency_canonical_digest" == "$record_dependency_canonical_digest" && "$manifest_receipt_digest" =~ ^[a-f0-9]{64}$ && "$manifest_receipt_checksum_digest" =~ ^[a-f0-9]{64}$ && "$manifest_receipt_cms_digest" =~ ^[a-f0-9]{64}$ ]] || die 'Gate 14 Gate 13 manifest source, config, toolchain, matrix, dependency, or receipt binding is incomplete.' 69
  /usr/bin/jq -e --arg cert "$(lowercase "$signing_certificate_sha256")" --arg fingerprint "$signing_fingerprint" --arg team "$signing_team_id" --arg identity "$signing_identity" '(.cmsSigner.certificateSHA256==($cert|ascii_downcase) and .cmsSigner.fingerprint==$fingerprint and .cmsSigner.teamID==$team and .cmsSigner.identity==$identity and (.sourceDigest|test("^[a-f0-9]{64}$")) and (.configDigest|test("^[a-f0-9]{64}$")) and (.toolchainDigest|test("^[a-f0-9]{64}$")) and (.matrixDigest|test("^[a-f0-9]{64}$")) and (.dependencyEvidenceDigest|test("^[a-f0-9]{64}$")) and (.dependencyEvidenceCanonicalDigest|test("^[a-f0-9]{64}$")) and (.phase08CompletionReceiptDigest|test("^[a-f0-9]{64}$")) and (.phase08CompletionReceiptChecksumDigest|test("^[a-f0-9]{64}$")) and (.phase08CompletionReceiptCMSDigest|test("^[a-f0-9]{64}$")))' "$dep_root/manifest-v1.json" >/dev/null || die 'Gate 14 Gate 13 manifest signer or digest binding is invalid.' 69
  /usr/bin/jq -e --arg gate13Matrix "$(sha "$gate13_matrix_path")" --arg expectedSource "$record_source" '(.matrixDigest==$gate13Matrix and .sourceCommit==$expectedSource)' "$dep_root/manifest-v1.json" >/dev/null || die 'Gate 14 Gate 13 manifest matrix or source commit binding is invalid.' 69
  actual_dependency_digest="$(sha "$dep_root/dependency-evidence-v1.json")"; actual_dependency_canonical_digest="$(canonical_json_digest "$dep_root/dependency-evidence-v1.json")"
  [[ "$actual_dependency_digest" == "$record_dependency_digest" && "$actual_dependency_digest" == "$manifest_dependency_digest" && "$actual_dependency_canonical_digest" == "$record_dependency_canonical_digest" && "$actual_dependency_canonical_digest" == "$manifest_dependency_canonical_digest" ]] || die 'Gate 14 actual Gate 13 dependency descriptor digest does not match its signed claims.' 69
  [[ "$manifest_receipt_digest" == "$(sha "$dep_root/phase08-completion-receipt-v1.json")" && "$manifest_receipt_checksum_digest" == "$(sha "$dep_root/phase08-completion-receipt-v1.sha256")" && "$manifest_receipt_cms_digest" == "$(sha "$dep_root/phase08-completion-receipt-v1.cms")" ]] || die 'Gate 14 Gate 13 receipt sidecar binding is incomplete or changed.' 69
  validate_gate13_transitive_descriptor "$dep_root/dependency-evidence-v1.json" || die 'Gate 14 signed Gate 13 dependency evidence is not a complete Gates 8-12 descriptor.' 69
  validate_gate13_receipt_file "$dep_root/phase08-completion-receipt-v1.json" "$dep_root/phase08-completion-receipt-v1.sha256" "$dep_root/phase08-completion-receipt-v1.cms" || die 'Gate 14 Gate 13 completion receipt binding is not cryptographically verified.' 69
  validate_gate13_inventory "$dep_root" || die 'Gate 14 Gate 13 dependency inventory is incomplete, unexpected, or tampered.' 69
  verify_cms_binding "$dep_root" || die 'Gate 14 Gate 13 CMS verification failed.' 69
}

validate_gate13_transitive_descriptor() {
  local file="$1"
  /usr/bin/jq -e --arg cert "$(lowercase "$signing_certificate_sha256")" --arg fingerprint "$signing_fingerprint" --arg team "$signing_team_id" --arg identity "$signing_identity" --arg source "$source_commit" \
'type=="object" and (keys|sort)==["records","schema"] and .schema=="hostwright.phase09.dependency-evidence.v1" and (.records|type)=="array" and (.records|length)==5 and ([.records[].gate]|sort)==[8,9,10,11,12] and all(.records[]; (keys|sort)==["checksumManifestDigest","cmsCertificateSHA256","cmsDigest","cmsFingerprint","cmsIdentity","cmsTeamID","configDigest","gate","manifestDigest","rootBasename","sourceCommit","sourceDigest","status","toolchainDigest"] and .status=="passed" and .sourceCommit==$source and (.rootBasename|test("^phase09-gate(08|09|10|11|12)-[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$")) and (.sourceDigest|test("^[a-f0-9]{64}$")) and (.configDigest|test("^[a-f0-9]{64}$")) and (.toolchainDigest|test("^[a-f0-9]{64}$")) and (.manifestDigest|test("^[a-f0-9]{64}$")) and (.checksumManifestDigest|test("^[a-f0-9]{64}$")) and (.cmsDigest|test("^[a-f0-9]{64}$")) and .cmsCertificateSHA256==($cert|ascii_downcase) and .cmsFingerprint==$fingerprint and .cmsTeamID==$team and .cmsIdentity==$identity)' "$file" >/dev/null
}

validate_dependency_file() {
  local file="$1" record
  private_file_0600 "$file" || die 'Gate 14 dependency evidence must be a canonical private regular file.' 69
  /usr/bin/jq -e 'type=="object" and (keys|sort)==["records","schema"] and .schema=="hostwright.phase09.dependency-evidence.v1" and (.records|type)=="array" and (.records|length)==1' "$file" >/dev/null || die 'Gate 14 dependency evidence must contain exactly one Gate 13 record.' 69
  record="$(/usr/bin/jq -c '.records[0]' "$file")"
  validate_dependency_record "$record"
}

validate_gate13_receipt_copy() {
  local record root_basename dep_root
  private_file_0600 "$root/dependency-evidence-v1.json" || return 1
  record="$(/usr/bin/jq -c '.records[0]' "$root/dependency-evidence-v1.json")"
  root_basename="$(/usr/bin/jq -r '.rootBasename' <<<"$record")"
  dep_root="${evidence_parent}/${root_basename}"
  validate_gate13_receipt_file "$dep_root/phase08-completion-receipt-v1.json" \
    "$dep_root/phase08-completion-receipt-v1.sha256" \
    "$dep_root/phase08-completion-receipt-v1.cms"
}

validate_formal_prerequisites() {
  evidence_parent="$(/bin/realpath "$(dependency_parent)")"
  [[ -d "$evidence_parent" && ! -L "$evidence_parent" ]] || die 'Gate 14 evidence dependency parent is unavailable.' 69
  local dependency_file="${HOSTWRIGHT_PHASE09_GATE14_DEPENDENCY_EVIDENCE:-}"
  [[ -n "$dependency_file" ]] || die 'Gate 14 requires HOSTWRIGHT_PHASE09_GATE14_DEPENDENCY_EVIDENCE.' 69
  validate_dependency_file "$dependency_file"
  if [[ -f "$root/dependency-evidence-v1.json" ]]; then
    [[ "$(sha "$dependency_file")" == "$(sha "$root/dependency-evidence-v1.json")" ]] || die 'Gate 14 dependency evidence changed after preparation.' 73
    if [[ -e "$root/manifest-v1.json" || -L "$root/manifest-v1.json" ]]; then
      private_file_0600 "$root/manifest-v1.json" || die 'Gate 14 prepared manifest is unsafe; preserve this root.' 73
      [[ "$(sha "$root/dependency-evidence-v1.json")" == "$(/usr/bin/jq -r '.dependencyEvidenceDigest' "$root/manifest-v1.json")" && "$(canonical_json_digest "$root/dependency-evidence-v1.json")" == "$(/usr/bin/jq -r '.dependencyEvidenceCanonicalDigest' "$root/manifest-v1.json")" ]] || die 'Gate 14 manifest dependency bindings changed after preparation.' 73
    fi
  fi
}

write_prepared_evidence() {
  atomic_copy "$HOSTWRIGHT_PHASE09_GATE14_DEPENDENCY_EVIDENCE" "$root/dependency-evidence-v1.json" || die 'Gate 14 dependency evidence copy failed closed.' 73
  printf '%s\n' "$state_header" | atomic_write_from_stdin "$root/state-v1.tsv" || die 'Gate 14 state preparation write failed closed.' 73
  printf '%s\n' "$ownership_header" | atomic_write_from_stdin "$root/ownership-v1.tsv" || die 'Gate 14 ownership preparation write failed closed.' 73
  toolchain_snapshot | atomic_write_from_stdin "$root/toolchain-v1.txt" || die 'Gate 14 toolchain preparation write failed closed.' 73
  /usr/bin/jq -n \
    ...
    --arg deps "$(sha "$root/dependency-evidence-v1.json")" --arg depsCanonical "$(canonical_json_digest "$root/dependency-evidence-v1.json")" --arg prepared "$(now)" \
    --arg testMode "$(testing && printf true || printf false)" --arg cert "$signing_certificate_sha256" --arg fingerprint "$signing_fingerprint" --arg team "$signing_team_id" --arg identity "$signing_identity" \
    '{schema:$schema,gate:$gate,status:"prepared",testMode:($testMode=="true"),preparedAt:$prepared,completedAt:null,
      ...
      dependencyEvidenceDigest:$deps,dependencyEvidenceCanonicalDigest:$depsCanonical,cellOrder:[1,2,3,4,5,6],evidenceClasses:["U","I","L","M","S","R"],
      productTestBaseline:500,formalClaim:false,cmsSigner:{certificateSHA256:$cert,fingerprint:$fingerprint,teamID:$team,identity:$identity}}' \
    | atomic_write_from_stdin "$root/manifest-v1.json" || die 'Gate 14 manifest preparation write failed closed.' 73
}

expected_inventory() {
  local mode="${1:-passed}" cell
  printf '%s\n' manifest-v1.json dependency-evidence-v1.json state-v1.tsv ownership-v1.tsv toolchain-v1.txt
...
validate_manifest_shape() {
  ...
  /usr/bin/jq -e --arg schema "$schema" --argjson gate "$gate" --arg source "$source_commit" --argjson testMode "$test_mode" --arg cert "$(lowercase "$signing_certificate_sha256")" --arg fingerprint "$signing_fingerprint" --arg team "$signing_team_id" --arg identity "$signing_identity" \
    'type=="object" and (keys|sort)==["cellOrder","cmsSigner","completedAt","configDigest","dependencyEvidenceCanonicalDigest","dependencyEvidenceDigest","evidenceClasses","formalClaim","gate","matrixDigest","preparedAt","productTestBaseline","schema","sourceCommit","sourceDigest","status","testMode","toolchainDigest"] and .schema==$schema and .gate==$gate and (.status=="prepared" or .status=="passed") and .sourceCommit==$source and (.sourceCommit|test("^[0-9a-f]{40}$")) and .testMode==$testMode and .productTestBaseline==500 and ((.status=="prepared" and .formalClaim==false) or (.status=="passed" and .formalClaim==(.testMode|not))) and .cmsSigner=={certificateSHA256:$cert,fingerprint:$fingerprint,teamID:$team,identity:$identity}' "$root/manifest-v1.json" >/dev/null
}

validate_prepared() {
  local status
  private_file_0600 "$root/manifest-v1.json" || die 'Gate 14 evidence root is not prepared or manifest is not private.' 73
  status="$(/usr/bin/jq -r '.status' "$root/manifest-v1.json")"
  case "$status" in
    prepared)
      [[ ! -e "$root/evidence-v1.sha256" && ! -L "$root/evidence-v1.sha256" && ! -e "$root/evidence-v1.cms" && ! -L "$root/evidence-v1.cms" ]] || die 'Gate 14 prepared evidence has an incomplete final seal; preserve this root.' 73
      ;;
    passed)
      private_file_0600 "$root/evidence-v1.sha256" && private_file_0600 "$root/evidence-v1.cms" || die 'Gate 14 passed evidence is missing one or both final seal files; preserve this root.' 73
      ;;
    failed)
      die 'Gate 14 evidence is frozen after failure; preserve this root.' 73
      ;;
    *)
      die 'Gate 14 manifest status is not runnable; preserve this root.' 73
      ;;
  esac
  validate_manifest_shape || die 'Gate 14 manifest schema, mode, signer, or baseline shape is invalid.' 73
  [[ "$(/usr/bin/jq -r '.sourceDigest' "$root/manifest-v1.json")" == "$source_digest_value" && "$(/usr/bin/jq -r '.configDigest' "$root/manifest-v1.json")" == "$config_digest_value" && "$(/usr/bin/jq -r '.toolchainDigest' "$root/manifest-v1.json")" == "$toolchain_digest_value" && "$(/usr/bin/jq -r '.matrixDigest' "$root/manifest-v1.json")" == "$(matrix_digest)" ]] || die 'Gate 14 prepared evidence dependencies changed; preserve this root.' 73
  private_file_0600 "$root/dependency-evidence-v1.json" || die 'Gate 14 prerequisite evidence is not private; preserve this root.' 73
  [[ "$(sha "$root/dependency-evidence-v1.json")" == "$(/usr/bin/jq -r '.dependencyEvidenceDigest' "$root/manifest-v1.json")" && "$(canonical_json_digest "$root/dependency-evidence-v1.json")" == "$(/usr/bin/jq -r '.dependencyEvidenceCanonicalDigest' "$root/manifest-v1.json")" ]] || die 'Gate 14 prerequisite evidence changed; preserve this root.' 73
  validate_formal_prerequisites
  validate_dependency_file "$root/dependency-evidence-v1.json"
}

revalidate_dependencies() {
  local dependency_file="${HOSTWRIGHT_PHASE09_GATE14_DEPENDENCY_EVIDENCE:-}" current_dependency_digest current_dependency_canonical_digest
  [[ "$(source_digest)" == "$source_digest_value" ]] || die 'Gate 14 source digest changed during qualification; preserve this root.' 73
  [[ "$(config_digest)" == "$config_digest_value" ]] || die 'Gate 14 configuration digest changed during qualification; preserve this root.' 73
  [[ "$(toolchain_digest)" == "$toolchain_digest_value" ]] || die 'Gate 14 toolchain digest changed during qualification; preserve this root.' 73
  validate_formal_prerequisites
  current_dependency_digest="$(sha "$dependency_file")"
  current_dependency_canonical_digest="$(canonical_json_digest "$dependency_file")"
  [[ -n "$dependency_digest_value" && "$current_dependency_digest" == "$dependency_digest_value" && "$current_dependency_canonical_digest" == "$dependency_canonical_digest_value" ]] || die 'Gate 14 dependency digest changed during qualification; preserve this root.' 73
  [[ -f "$root/dependency-evidence-v1.json" && "$current_dependency_digest" == "$(sha "$root/dependency-evidence-v1.json")" && "$current_dependency_canonical_digest" == "$(canonical_json_digest "$root/dependency-evidence-v1.json")" ]] || die 'Gate 14 prepared dependency copy changed during qualification; preserve this root.' 73
}

matrix_filter() {
  /usr/bin/jq -r --argjson cell "$1" '[.cells[] | select(.cell == $cell) | .testcases[] | "\(.classname)/\(.name)"] | join("|")' "$matrix_path"
}

matrix_expected() {
  /usr/bin/jq -r --argjson cell "$1" '.cells[] | select(.cell == $cell) | .expectedTests' "$matrix_path"
}

validate_structured_results() {
  local result="$1" cell="$2" testcases expected
  testcases="$(/usr/bin/jq -c --argjson cell "$cell" '[.cells[]|select(.cell==$cell)|.testcases[]|"\(.classname)/\(.name)"]' "$matrix_path")"
  expected="$(matrix_expected "$cell")"
  private_file_0600 "$result" || return 1
  "$python_path" - "$result" "$testcases" "$expected" <<'PY'
import json, sys, xml.etree.ElementTree as ET
from collections import Counter
path, selectors, expected = sys.argv[1:]
raw = open(path, 'rb').read()
if b'<!DOCTYPE' in raw or b'<!ENTITY' in raw: raise SystemExit(1)
root = ET.fromstring(raw); wanted = Counter(json.loads(selectors)); seen = Counter(); cases = list(root.iter('testcase'))
for case in cases:
    if list(case): raise SystemExit(1)
    cls = case.attrib.get('classname', '').rsplit('.', 1)[-1]; name = case.attrib.get('name', '')
    ident = f'{cls}/{name}'
    if not cls or not name or not name.startswith('test') or ident not in wanted: raise SystemExit(1)
    seen[ident] += 1
...
cell_command() {
  case "$1" in
    1) printf '%s\n' 'swift test --jobs 1 --filter Gate14 strict contract and decoding selectors' ;;
    2) printf '%s\n' 'swift test --jobs 1 --filter Gate14 persistence transport and daemon integration selectors' ;;
    3) printf '%s\n' 'swift test --jobs 1 --filter Gate14 local lifecycle integration selectors; product builds' ;;
    4) printf '%s\n' 'swift test --jobs 1 --filter Gate14 schema 17 through 21 compatibility selectors' ;;
    5) printf '%s\n' 'swift test --jobs 1 --filter Gate14 aggregate security selectors' ;;
    6) printf '%s\n' 'swift test --jobs 1 --filter Gate14 recovery and resilience selectors; lint/docs/diff' ;;
    *) die 'unknown Gate 14 cell.' 70 ;;
  esac
}
...
run_cell() {
  ...
  filter="$(matrix_filter "$cell")"; expected="$(matrix_expected "$cell")"
  ...
  if ! atomic_harden "$candidate"; then
    dispose_private_artifact "$candidate" "$xunit_dir" || true
    die 'Gate 14 structured result hardening failed closed.' 74
  fi
  if ! private_file_0600 "$candidate"; then
    dispose_private_artifact "$candidate" "$xunit_dir" || true
    die 'Gate 14 test runner did not produce a private structured xUnit result.' 74
  fi
  if ! validate_structured_results "$candidate" "$cell" >/dev/null; then
    dispose_private_artifact "$candidate" "$xunit_dir" || true
    die "Gate 14 cell $cell structured selector evidence is invalid." 74
  fi
  final_result="$root/cell-$(printf '%02d' "$cell").xunit.xml"
  if ! atomic_copy "$candidate" "$final_result"; then
    dispose_private_artifact "$candidate" "$xunit_dir" || true
    die 'Gate 14 structured result publication failed closed.' 73
  fi
  dispose_private_artifact "$candidate" "$xunit_dir" || die 'Gate 14 xUnit temporary directory could not be disposed.' 73
  if [[ "$cell" == 3 ]]; then
    swift_exec build --jobs 1 --product hostwright
    swift_exec build --jobs 1 --product hostwrightd
  elif [[ "$cell" == 6 ]]; then
    swift_exec build --jobs 1 --product hostwright
    swift_exec build --jobs 1 --product hostwrightd
    if ! testing; then
...
run_diagnostic_cell() {
  ...
  filter="$(matrix_filter "$cell")"; expected="$(matrix_expected "$cell")"
  diagnostic_dir="$(/usr/bin/mktemp -d -t hostwright-phase09-gate14-diagnose)"
...
diagnose() {
  ...
  /usr/bin/jq -n --arg schema "$schema" --argjson gate "$gate" --arg claim none --arg status diagnostic \
    ...
    --argjson cells "[${cells}]" --argjson expected "$product_test_count" --argjson observed "$total" --arg exact "$exact" \
...
append_state() {
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' "$gate" "$1" "$2" "$source_digest_value" "$config_digest_value" "$toolchain_digest_value" "$3" "$4" "$5" "$6" "$7" | atomic_log_replace_from_stdin "$root/state-v1.tsv" || die 'Gate 14 state log replacement failed closed.' 74
}

append_failure() {
  if [[ ! -e "$root/failure-v1.tsv" && ! -L "$root/failure-v1.tsv" ]]; then
    printf '%s\n' $'recorded_at\tgate\tcell\texit_status\tcommand\tsource_digest\tconfig_digest\ttoolchain_digest\tstdout_sha256\tstderr_sha256\tstructured_result_sha256' | atomic_write_from_stdin "$root/failure-v1.tsv" || die 'Gate 14 failure ledger creation failed closed.' 74
  fi
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' "$(now)" "$gate" "$1" "$2" "$3" "$source_digest_value" "$config_digest_value" "$toolchain_digest_value" "$4" "$5" "$6" | atomic_log_replace_from_stdin "$root/failure-v1.tsv" || die 'Gate 14 failure ledger log replacement failed closed.' 74
}
...
verify_reusable() {
  ...
  for cell in 1 2 3 4 5 6; do
    ...
    private_file_0600 "$stdout_log" && private_file_0600 "$stderr_log" && private_file_0600 "$xunit_log" || return 1
    ...
    expected_out="$(/usr/bin/awk -F $'\t' -v cell="$cell" '$2==cell&&$3=="pass"{x=$9}END{print x}' "$root/state-v1.tsv")"; expected_err="$(/usr/bin/awk -F $'\t' -v cell="$cell" '$2==cell&&$3=="pass"{x=$10}END{print x}' "$root/state-v1.tsv")"; expected_xunit="$(/usr/bin/awk -F $'\t' -v cell="$cell" '$2==cell&&$3=="pass"{x=$11}END{print x}' "$root/state-v1.tsv")"
...
write_evidence() {
  ...
  revalidate_dependencies
  for file in "$root/.evidence-v1.sha256.tmp" "$root/.evidence-v1.cms.tmp" "$root/.evidence-v1.decoded.tmp" "$root/.manifest-v1.passed.tmp"; do assert_absent "$file" || die 'Gate 14 fixed sealing temporary already exists or is symlinked; preserve this root.' 73; done
  assert_absent "$root/gate-active-run-v1-info.tsv" || die 'Gate 14 active-run information destination already exists or is symlinked.' 73
  validate_formal_prerequisites
  validate_gate13_receipt_copy || die 'Gate 14 Gate 13 receipt was not revalidated before sealing.' 73
  atomic_publish "$parent/.phase09-gate14-active-v1/info-v1.tsv" "$root/gate-active-run-v1-info.tsv" || die 'Gate 14 active-run inventory publication failed closed.' 73
  ...
  { printf '%s  manifest-v1.json\n' "$(sha "$manifest_tmp")"; for file in dependency-evidence-v1.json state-v1.tsv ownership-v1.tsv toolchain-v1.txt gate-active-run-v1-info.tsv cell-01.stdout.log cell-01.stderr.log cell-01.xunit.xml cell-02.stdout.log cell-02.stderr.log cell-02.xunit.xml cell-03.stdout.log cell-03.stderr.log cell-03.xunit.xml cell-04.stdout.log cell-04.stderr.log cell-04.xunit.xml cell-05.stdout.log cell-05.stderr.log cell-05.xunit.xml cell-06.stdout.log cell-06.stderr.log cell-06.xunit.xml; do private_file_0600 "$root/$file" || die "Gate 14 cannot seal missing or unsafe $file." 74; printf '%s  %s\n' "$(sha "$root/$file")" "$file"; done; } | LC_ALL=C /usr/bin/sort | atomic_replace_from_stdin "$digest"
  validate_staged_digest "$digest" "$manifest_tmp" || die 'Gate 14 staged checksum inventory is incomplete, duplicated, or changed.' 74
  if ! security_exec cms -S -N "$signing_identity" -H SHA256 -u 9 -i "$digest" -o "$cms"; then
    dispose_private_artifact "$cms" "$cms_dir" || true
    die 'Gate 14 CMS evidence signing failed.' 74
  fi
  if ! atomic_harden "$cms"; then
    dispose_private_artifact "$cms" "$cms_dir" || true
    die 'Gate 14 CMS output hardening failed closed.' 74
  fi
  if ! private_file_0600 "$cms"; then
    dispose_private_artifact "$cms" "$cms_dir" || true
    die 'Gate 14 CMS output identity or mode changed.' 74
  fi
  if ! verify_cms_payload "$cms" "$digest" final; then
    dispose_private_artifact "$cms" "$cms_dir" || true
    die 'Gate 14 staged CMS evidence did not round-trip with the pinned signer.' 74
  fi
  assert_absent "$root/evidence-v1.sha256" && assert_absent "$root/evidence-v1.cms" || die 'Gate 14 final evidence destination already exists or is symlinked.' 73
  private_file_0600 "$root/manifest-v1.json" || die 'Gate 14 prepared manifest is unsafe at finalization.' 73
  if ! atomic_copy "$digest" "$root/evidence-v1.sha256"; then
    dispose_private_artifact "$cms" "$cms_dir" || true
    die 'Gate 14 final checksum publication failed closed.' 73
  fi
  if ! atomic_copy "$cms" "$root/evidence-v1.cms"; then
    dispose_private_artifact "$cms" "$cms_dir" || true
    die 'Gate 14 final CMS publication failed closed.' 73
  fi
  if ! /bin/cat "$manifest_tmp" | atomic_replace_from_stdin "$root/manifest-v1.json"; then
    dispose_private_artifact "$cms" "$cms_dir" || true
    die 'Gate 14 final manifest publication failed closed.' 73
  fi
  /bin/unlink "$manifest_tmp"
  /bin/unlink "$digest"
  dispose_private_artifact "$cms" "$cms_dir" || die 'Gate 14 CMS external artifact could not be disposed.' 73
  validate_checksum_manifest "$root/evidence-v1.sha256" "$root" exact || die 'Gate 14 final checksum inventory failed verification.' 74
  verify_cms_payload "$root/evidence-v1.cms" "$root/evidence-v1.sha256" final || die 'Gate 14 final CMS verification failed.' 74
  revalidate_dependencies
  validate_prepared || die 'Gate 14 final manifest, dependency, or Gate 13 receipt binding failed revalidation.' 74
  /bin/rmdir "$root/active-run-v1" || die 'Gate 14 active root lock was not empty at finalization.' 74
  if ! compare_inventory passed; then /bin/mkdir "$root/active-run-v1"; /bin/chmod 700 "$root/active-run-v1"; die 'Gate 14 final evidence inventory is incomplete or changed.' 74; fi
}

release_locks() {
  if [[ "$gate_lock_created" == 1 ]]; then
    /bin/rmdir "$parent/.phase09-gate14-active-v1"
...
on_exit() {
  ...
  if [[ "$status" == 0 && "$run_succeeded" == 1 ]]; then release_locks || status=$?; fi
  exit "$status"
}

run() {
  validate_prepared
  if [[ -e "$root/evidence-v1.sha256" || -L "$root/evidence-v1.sha256" || -e "$root/evidence-v1.cms" || -L "$root/evidence-v1.cms" ]]; then
    revalidate_dependencies
    verify_reusable || die 'Gate 14 completed evidence is incomplete or changed; preserve this root and do not rerun.' 73
    printf '%s\n' 'Gate 14 evidence is valid and reused; no cells were rerun.'; return 0
  fi
  local lock="$parent/.phase09-gate14-active-v1" cell command stdout_log stderr_log xunit_log started finished status stdout_sha stderr_sha xunit_sha
  [[ ! -e "$lock" && ! -L "$lock" && ! -e "$root/active-run-v1" && ! -L "$root/active-run-v1" ]] || die 'An active Gate 14 qualification already exists; do not duplicate it.' 75
  for cell in 1 2 3 4 5 6; do
    stdout_log="$root/cell-$(printf '%02d' "$cell").stdout.log"; stderr_log="$root/cell-$(printf '%02d' "$cell").stderr.log"; xunit_log="$root/cell-$(printf '%02d' "$cell").xunit.xml"
  ...
  printf '%s\n' $'root\tpid\tstarted_at\tsource_digest\tconfig_digest\ttoolchain_digest' | atomic_write_from_stdin "$lock/info-v1.tsv" || die 'Gate 14 lock inventory creation failed closed.' 74
  printf '%s\t%s\t%s\t%s\t%s\t%s\n' "$root" "$$" "$(now)" "$source_digest_value" "$config_digest_value" "$toolchain_digest_value" | atomic_log_replace_from_stdin "$lock/info-v1.tsv" || die 'Gate 14 lock inventory log replacement failed closed.' 74
  gate_lock_created=1
  /bin/mkdir "$root/active-run-v1"; /bin/chmod 700 "$root/active-run-v1"; root_lock_created=1
  run_active=1
  trap on_exit EXIT
  for cell in 1 2 3 4 5 6; do
    revalidate_dependencies; command="$(cell_command "$cell")"
    stdout_log="$root/cell-$(printf '%02d' "$cell").stdout.log"; stderr_log="$root/cell-$(printf '%02d' "$cell").stderr.log"; xunit_log="$root/cell-$(printf '%02d' "$cell").xunit.xml"; started="$(now)"
    set +e; HOSTWRIGHT_PHASE09_INTERNAL_CELL_RUN=1 atomic_run_outputs "$stdout_log" "$stderr_log" "$bash_path" "$harness_path" cell "$cell"; status=$?; set -e
    ...
    if [[ "$status" != 0 ]]; then
      append_state "$cell" failed "$started" "$finished" "$stdout_sha" "$stderr_sha" "$xunit_sha"; append_failure "$cell" "$status" "$command" "$stdout_sha" "$stderr_sha" "$xunit_sha"; mark_failed
      die "Gate 14 cell $cell failed; progress is frozen and both locks are preserved." "$status"
    fi
    append_state "$cell" pass "$started" "$finished" "$stdout_sha" "$stderr_sha" "$xunit_sha"
  done
  write_evidence "$(now)"; run_succeeded=1
  printf '%s\n' 'Gate 14 qualification passed.'
}

prepare() {
  validate_root; empty_root; collect; evidence_parent="$(/bin/realpath "$(dependency_parent)")"; validate_formal_prerequisites; write_prepared_evidence
  compare_inventory prepared || die 'Gate 14 prepared evidence inventory is incomplete or changed.' 74
  printf '%s\n' 'Gate 14 evidence root prepared.'
}

main() {
  [[ "$#" -ge 1 ]] || die 'usage: phase09-gate14-qualification.sh <contract|diagnose|prepare|run> [args].' 64
  case "$1" in
    ...
    prepare) [[ "$#" == 2 && "$2" == 14 ]] || die 'usage: prepare 14.' 64; validate_worktree; cd "$repository_root"; validate_root; prepare ;;
    run) [[ "$#" == 2 && "$2" == 14 ]] || die 'usage: run 14.' 64; validate_worktree; cd "$repository_root"; validate_root; collect; run ;;
    cell) [[ "${HOSTWRIGHT_PHASE09_INTERNAL_CELL_RUN:-}" == 1 && "$#" == 2 && "$2" =~ ^[1-6]$ ]] || die 'unknown Gate 14 cell.' 64; validate_worktree; cd "$repository_root"; validate_root; collect; run_cell "$2" ;;
    *) die 'unknown Gate 14 qualification command.' 64 ;;
  esac
}
