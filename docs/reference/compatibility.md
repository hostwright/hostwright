...
| Manifest | Strict bounded Yams 6.2.2 decoding, canonical Manifest v2 encoding, complete executable Phase 04 single-host and qualified Phase 07 networking fields, and deterministic legacy-health migration. Unknown or unsupported fields fail before mutation. | Repeat parser, migration, and executable-field qualification at GA. |
| Lifecycle | Confirmed single-host dependency/replica lifecycle, typed probes, bounded interactive operations, rolling/recreate updates, verified rollback, and resumable recovery through one schema-v16 saga. Images must already exist locally and are bound to reviewed descriptor/platform-variant locks. | Repeat current/previous provider and destructive-recovery qualification at GA. |
| Daemon | Foreground and exact current-user LaunchAgent modes use the same runtime-read-only loop. The LaunchAgent lifecycle is implemented for one macOS user and has isolated install/crash/upgrade/rollback/disable/repair/uninstall evidence; reboot and logout/login remain an attended Phase 08 Gate 1 blocker. | Repeat the complete lifecycle on the supported macOS matrix and pass later unattended-policy/soak gates before GA. |
...
| State | SQLite schema v16 for standalone/node-local state, including immutable image digest locks, verified OCI referrer graph evidence, image-trust audit, exact image-SBOM records, signed vulnerability-report decisions/exceptions, exact signed build-provenance evidence, provider-scoped content accounting, exact storage volume/snapshot/backup/orphan/quota records, and UUID/fence-bound network authority. | Qualified backup/restore/repair and etcd 3.7.x authority for multi-Mac clusters. |
