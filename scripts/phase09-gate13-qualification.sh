...
config_digest() {
  local file
  {
    for file in "$matrix_path" "scripts/phase09-gate13-qualification.sh" "Tests/HostwrightStateTests/Phase09Gate13QualificationHarnessTests.swift"; do
...
toolchain_snapshot() {
  if testing; then
    ...
    [[ "$resolved_swift" == "$swift_path" && "$resolved_python" == "$python_path" ]] || die 'Gate 13 xcrun-resolved formal tools do not match the pinned system paths.' 69
...
run_cell() {
  ...
  set +e
  HOSTWRIGHT_PHASE09_EXPECTED_TESTS="$expected" HOSTWRIGHT_PHASE09_EXPECTED_SELECTORS_JSON="$(/usr/bin/jq -c --argjson cell "$cell" '[.tests[]|select(.cell==$cell)|.selector]' "$matrix_path")" \
...
run_diagnostic_cell() {
  ...
  set +e
  output="$(HOSTWRIGHT_PHASE09_EXPECTED_TESTS="$expected" HOSTWRIGHT_PHASE09_EXPECTED_SELECTORS_JSON="$(/usr/bin/jq -c --argjson cell "$cell" '[.tests[]|select(.cell==$cell)|.selector]' "$matrix_path")" swift_exec test --jobs 1 --filter "$filter" --xunit-output "$xunit_base" 2>&1)"
  diagnostic_status=$?
  set -e
