...
7. Completed for the first bounded slice: implement the reviewed-local executable extension handshake host. Continue in order with the local control API, separate GUI, local secure-exposure provider, host-native accelerator path, exact-version compatibility work, and multi-host proof.
