...
readonly formal_harness_path='/Users/dev/Documents/hostwright-phase09/scripts/phase09-gate13-qualification.sh'
...
validate_worktree() {
  ...
  if testing; then
    ...
    [[ -f "$invocation" && ! -L "$invocation" ]] || die 'Gate 13 test script invocation is unsafe.' 66
    canonical="$(/bin/realpath "$invocation")" || die 'Gate 13 test script cannot be resolved.' 66
    repository_root="$(/bin/realpath "$(dirname "$canonical")/..")" || die 'Gate 13 test repository cannot be resolved.' 66
    harness_path="$canonical"
    [[ "$harness_path" == "$repository_root/scripts/phase09-gate13-qualification.sh" \
      && "$repository_root" != '/Users/dev/Documents/hostwright' ]] \
      || die 'Gate 13 test script is outside its invoking repository.' 66
    top="$(cd "$repository_root" && "$git_path" rev-parse --show-toplevel)"
    [[ "$(/bin/realpath "$top")" == "$repository_root" ]] || die 'Gate 13 test repository identity changed.' 66
    return
  fi
  top="$(cd "$repository_root" && "$git_path" rev-parse --show-toplevel)"
  ...
  [[ "$top" == "$formal_repository_root" ]] || die 'Gate 13 requires the isolated Phase 09 worktree.' 66
}
