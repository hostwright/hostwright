...
| `up`, `down`, `run`, `start`, `stop`, `restart`, `rm`, `update` | Matching lifecycle command with `--output json` | Requires exactly one dry-run or exact plan confirmation and delegates to the shared schema-v16 lifecycle saga with provider/platform-bound image, storage, and network authority. |
