...
`doctor` does not run Apple container commands. It does not inspect containers, networks, volumes, images, logs, or runtime state. Runtime observation exists only behind `RuntimeAdapter`; doctor remains a diagnostic executable-presence check plus local policy reporting.
