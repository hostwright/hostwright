...
Phase 08 Gates 11–15 provide structured OSLog, durable local event cursors and bounded long-poll watches, a fixed-cardinality schema-v1 metrics/SLO projection, bounded correlated local traces, and consent-bound privacy-safe support bundles. Metrics and traces read or append only through retained schema-v17 authority and expose confirmation-bound private file exports. Support bundles collect only fixed bounded redacted projections, can encrypt in memory through macOS CMS, journal file effects, and require retained ownership evidence for exact deletion. None adds a listener or automatic upload; see [Bounded Local Metrics and SLOs](metrics.md), [Correlated Local Traces](traces.md), and [Privacy-Safe Support Bundles](support-bundles.md). Aggregate soak qualification remains owned by Gate 16, so the `observability.telemetry` capability remains `experimental` until that evidence passes.
