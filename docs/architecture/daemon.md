...
Those events and daemon operation records are local forensic inputs for `hostwright events`, `hostwright recovery`, and `hostwright diagnostics`. After a durable event transaction commits, a bounded structured mirror is emitted under the `dev.hostwright` OSLog subsystem with the daemon-run correlation identifier. The mirror contains fixed metadata rather than event messages or payloads; it is not authoritative and is never exported or uploaded. See [Local Observability](../reference/observability.md).
