...
Trace-span rows have the independent Manifest v3 `traces` retention class. Ordinary `events` and `audits` selection excludes them, while `hostwright events` may still display their durable rows. Compaction remains confirmation-bound, creates a verified backup, honors holds and recovery horizons, and deletes only exact eligible trace event identities.
