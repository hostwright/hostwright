...
| HW-TEAM-003, HW-OBS-001, HW-SAFE-004 | Profile, manifest, plan, and approval hashes persist through runtime confirmations and redacted append-only event/operation records using explicit state paths. | Automated + local integration | Real SQLite apply/cleanup audit tests and state redaction tests. |
...
| Requirement IDs | Acceptance criteria | Verification type | Verification command or review |
| --- | --- | --- | --- |
| HW-GUI-005, HW-SAFE-004 | API version 1 accepts exactly one bounded JSON request over a real pipe, emits exactly one bounded JSON response, and requires launch-fixed absolute manifest/state/profile paths with no default state path. | Unit contract + local integration | `ControlRequestTests`, real-pipe timeout/overflow/EOF cases, and `scripts/integration.sh`. |
