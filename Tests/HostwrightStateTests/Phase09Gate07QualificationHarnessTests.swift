...
final class Phase09Gate07QualificationHarnessTests: XCTestCase {
  ...
  private var harness: URL {
    repository.appendingPathComponent("scripts/phase09-gate07-qualification.sh")
  }

  func testContractFixesGateAndAllSixProfileEvidenceCells() throws {
    let result = try run(["contract"])
    XCTAssertEqual(result.status, 0, result.stderr)
    XCTAssertTrue(result.stdout.contains("Gate 7 — 43.75%"))
    XCTAssertTrue(result.stdout.contains("Exactly one Gate 7 qualification"))
    XCTAssertTrue(result.stdout.contains("profile-bound pinned Apple-container live workload"))
    let source = try String(contentsOf: harness, encoding: .utf8)
    XCTAssertTrue(source.contains("for n in 1 2 3 4 5 6"))
    XCTAssertTrue(source.contains("WorkloadProfileRepositoryTests|WorkloadProfilePolicyEngineTests"))
    XCTAssertTrue(source.contains("WorkloadProfileControlOperationsTests|PersistentControlAdmissionIntegrationTests|LocalControlProfileIntegrationTests|LifecycleProfileIntegrationTests"))
    XCTAssertTrue(source.contains("hostwright-profile-qualification"))
    XCTAssertTrue(source.contains("--probe-apple-container"))
    XCTAssertTrue(source.contains("RBACSchemaV20MigrationTests"))
    XCTAssertTrue(source.contains("testWeakeningRequiresExactRbacApprovedFutureApproval"))
    XCTAssertTrue(source.contains("testProviderCapabilityDesiredWorkloadAndDriftFailuresAreExplicit"))
    XCTAssertTrue(source.contains("testTamperedCanonicalJSONAndDigestFailClosed"))
    XCTAssertTrue(source.contains("container run --rm --name"))
  ...
  func testPrepareBindsPrivateRootToGateSevenDependenciesAndLedger() throws {
    try withRoot { root, environment in
      let result = try run(["prepare", "7"], environment: environment)
      XCTAssertEqual(result.status, 0, result.stderr)
      let manifest = try JSONSerialization.jsonObject(
        with: Data(contentsOf: root.appendingPathComponent("manifest-v1.json"))) as? [String: Any]
      XCTAssertEqual(manifest?["schema"] as? String, "hostwright.phase09.gate07.qualification.manifest.v1")
      XCTAssertEqual(manifest?["gate"] as? Int, 7)
  ...
  func testWrongGateAndProtectedWorktreeAreRejected() throws {
    ...
    XCTAssertTrue(wrongGate.stderr.contains("only prepare 7"))

    try withRoot { _, environment in
      let protected = try run(
        ["prepare", "7"], environment: environment,
  ...
  func testFailurePreservesLocksAndDoesNotRunNextCell() throws {
    try withRoot { root, environment in
      XCTAssertEqual(try run(["prepare", "7"], environment: environment).status, 0)
      ...
      let failed = try run(["run", "7"], environment: failing)
      ...
      XCTAssertTrue(FileManager.default.fileExists(
        atPath: root.deletingLastPathComponent().appendingPathComponent(".phase09-gate07-active-v1").path))
  ...
  private func withRoot(_ body: (URL, [String: String]) throws -> Void) throws {
    ...
    let root = parent.appendingPathComponent("phase09-gate07-\(UUID().uuidString.lowercased())", isDirectory: true)
  ...
  private func canonicalPath(_ url: URL) throws -> String {
    ...
    try process.run(); process.waitUntilExit()
    guard process.terminationStatus == 0 else { throw NSError(domain: "Gate07Harness", code: 1) }
