...
validate_worktree(){ local top; top="$(/bin/realpath "$(git rev-parse --show-toplevel)")"; if testing; then [[ "$top" != /Users/dev/Documents/hostwright ]] || die "Gate 9 requires branch $readonly_branch." 66; return; fi; [[ "$(git branch --show-current)" == "$readonly_branch" ]] || die "Gate 9 requires branch $readonly_branch." 66; [[ "$top" == /Users/dev/Documents/hostwright-phase09 ]] || die 'Gate 9 requires the isolated Phase 09 worktree.' 66; }
...
record_process(){ local pid="$1" executable="$2" command start sample_n; kill -0 "$pid" 2>/dev/null || die 'owned daemon process is not alive.'; for sample_n in $(/usr/bin/seq 1 100); do command="$(ps -p "$pid" -o command= 2>/dev/null || true)"; start="$(ps -p "$pid" -o lstart= 2>/dev/null || true)"; [[ "${command%% *}" == "$executable" && -n "$start" ]] && break; command=''; start=''; /bin/sleep 0.1; done; [[ -n "$command" && "${command%% *}" == "$executable" && -n "$start" ]] || die 'owned daemon process identity is incomplete.'; printf '%s\tprocess\thostwrightd\t%s\t%s\t%s\tpid=%s;command_sha256=%s;start_sha256=%s;scope=gate09-live\n' "$(now)" "$executable" "$(stat -f '%d' "$executable")" "$(stat -f '%i' "$executable")" "$pid" "$(printf '%s' "$command"|stream_sha)" "$(printf '%s' "$start"|stream_sha)" >> "$root/ownership-v1.tsv"; }
...
start_daemon(){ local runtime="$1" executable="$2" config="$3" state="$4" generation="$5" cli="$6" socket="$runtime/app-support/run/control-v2.sock" n; (( ${#socket} < 104 )) || die 'the Gate 9 control socket exceeds the macOS Unix-domain path limit.' 66; HOSTWRIGHT_APPLICATION_SUPPORT_DIR="$runtime/app-support" HOSTWRIGHT_CACHE_DIR="$runtime/cache" HOSTWRIGHT_LOG_DIR="$runtime/logs" "$executable" --foreground --config "$config" --state-db "$state" --interval 5 --jitter 0 --parallelism 1 >"$runtime/daemon.$generation.stdout.log" 2>"$runtime/daemon.$generation.stderr.log" & local pid=$!; record_process "$pid" "$executable"; for n in {1..240}; do if [[ -S "$socket" ]]; then wait_for_authenticated_daemon "$runtime" "$cli"; printf '%s\n' "$pid"; return; fi; if ! kill -0 "$pid" 2>/dev/null; then /bin/cat "$runtime/daemon.$generation.stdout.log" >&2; /bin/cat "$runtime/daemon.$generation.stderr.log" >&2; die 'owned daemon exited before publishing its socket.'; fi; /bin/sleep .25; done; die 'owned daemon did not publish its socket.' 124; }
forced_live_failure_for_testing(){ local runtime executable pid resource='hostwright-v2-p09-test-live'; runtime="$(short_live_runtime)"; mkdir "$runtime"; chmod 700 "$runtime"; record_root "$runtime"; executable="$runtime/signed-hostwrightd"; { printf '%s\n' '#!/bin/sh'; printf '%s\n' 'exec /bin/sleep 300'; } > "$executable"; chmod 700 "$executable"; printf '%s\n' 'forced Gate 9 live failure diagnostic' > "$runtime/daemon.forced.stderr.log"; /bin/bash -c 'exec -a "$0" /bin/sleep 300' "$executable" & pid=$!; record_process "$pid" "$executable"; container phase09-test-create "$resource"; record_container "$resource"; return 47; }
