...
| HW-SAFE-008 | Policy decisions must be deterministic, explainable, local, fail-closed for unknown or high-risk scope, and non-mutating. | Phase 32 maintainer scope | Implemented for local planner, cleanup, image, env/secret, lifecycle, untrusted-manifest, exposure, accelerator, and extension declaration policy decisions | `Sources/HostwrightPolicy/LocalPolicy.swift`, `Sources/HostwrightPolicy/ExtensionPolicy.swift`, `Sources/HostwrightReconciler/PlanningPolicy.swift`, `Sources/HostwrightCLI/CleanupCommand.swift`, `docs/reference/policy.md` | Policy, reconciler, CLI, and core docs XCTest cases | 32, 33 |
