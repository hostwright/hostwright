...
| 21 | API and GUI Readiness Gate | Requirements complete; bounded read API delivered in Phase 42; GUI not started | Define local GUI/control-surface requirements, data contracts, accessibility expectations, command/API boundaries, safety rules, and handoff criteria. | Requirements are documented; Phase 42 implements only a one-shot local API for five existing non-runtime-mutating command contracts, while GUI work remains future and separately gated. |
...
| 41 | Reviewed-Local Executable Extension Handshake Host | Complete locally | Execute one explicit reviewed-local, read-only, digest-bound protocol handshake without generic plugin loading or capability invocation. | Strict declarations, policy blockers, secure file staging, bounded subprocess behavior, exact response binding, cleanup, built-CLI output, and real compiled-process tests pass. |
| 42 | One-Shot Local Control API | Complete locally | Expose a narrow versioned local JSON contract for existing plan, status, events, recovery, and doctor behavior without a daemon service or mutation endpoint. | Strict request parsing, explicit launch paths, path safety checks, bounded one-request I/O, delegated CLI semantics, real files/SQLite/pipes, and built-tool subprocess checks pass. |
...
7. Completed for the first two bounded slices: implement the reviewed-local executable extension handshake host and one-shot local control API. Separate GUI work is the next control-surface track and still requires its explicit hard-stop approval; provider, accelerator, compatibility, and multi-host work remain separately evidence-gated.
...
- `hostwrightd --foreground` and the exact managed service mode level-trigger supported drift through the shared fenced lifecycle saga. Phase 08 policy, recovery, authority, retention, OSLog, event-watch, bounded local metrics, correlated local trace, and privacy-safe support-bundle gates are implemented; aggregate soak and final closure remain incomplete.
- Control-surface work now includes only `hostwright-control`, a one-shot local JSON process for plan, status, events, recovery, and doctor through existing CLI contracts. No GUI, web dashboard, daemon API, socket/HTTP listener, background service, request-selected file path, mutation operation, direct RuntimeAdapter/SQLite access, telemetry upload, hosted diagnostics, or bypass around existing Hostwright command gates exists.
...
- Extension execution is limited to `hostwright extension check` with explicit absolute declaration/executable paths, reviewed-local trust, one read-only policy-allowed capability, exact SHA-256 binding, private staged execution, bounded one-shot JSON handshake, and exact cleanup. Current core has no discovery, installation, registry, capability invocation, untrusted execution, runtime-mutation extension, state-write extension, networking provider, tunnel provider, secret backend extension, or accelerator extension.
- Team workflow is explicit local profile and approval data only: `HostwrightPolicy` accepts strict-only requirements, exact operation bindings, and append-only audit records through explicit state paths. Current core has no cloud team service, central remote control, hosted audit log, user tracking, enterprise support workflow, remote policy distribution, macOS user/group/ACL management, or shared-secret management.
- Stack-file import is conversion-only: it prints reviewed `hostwright.yaml` text for a narrow safe subset and rejects unsupported networking, secrets, configs, build, unsupported deploy, named-volume, shell-healthcheck, cloud, tunnel, and lifecycle semantics without writing files, touching state, observing runtime, pulling images, or claiming Compose parity.
...
- Rejected unsupported or unsafe stack fields including `build`, unsupported `deploy` fields, `depends_on`, networking/discovery, secrets, configs, env files, named volumes, shell health checks, and cloud/tunnel semantics.
...
- Added strict, versioned JSON team-profile and approval-record artifacts with unknown-field rejection and canonical SHA-256 hashes.
- Team profiles are loaded only from explicit paths by `validate`, `plan`, `import-stack`, `apply`, and `cleanup`; no default or discovery path exists.
- Operational profiles can only preserve core gates or add `requireImageDigest` and `requireManifestReview`; policy weakening is not representable.
- Profile-aware apply and confirmed cleanup require approval records bound to the exact profile hash, manifest hash, plan hash or cleanup token, and operation scope.
- Runtime confirmations, operation ledger payloads, ownership metadata, and redacted append-only events preserve profile, manifest, plan, and approval hashes without bypassing confirmation or ownership.
- XCTest uses real temporary files and real SQLite databases for profile loading and audit persistence; the built-CLI integration gate covers explicit profile files and no hidden state writes.
- `docs/reference/team-workflow.md` documents schemas, commands, review flow, audit records, shared-machine expectations, and non-goals.
...
- Added `HostwrightExtensions` with a strict flat declaration format for one reviewed-local capability, exact executable SHA-256, declaration and protocol version 1, purpose, kind, and required boundaries.
- Added `ReviewedLocalExtensionHost` for an explicit one-shot handshake only. It evaluates the existing extension policy before execution and rejects unsupported trust, capability, kind, boundary, or version combinations.
- Declaration and executable inputs must be caller-owned regular non-symlink files without group/world write permission. The executable is copied from an open descriptor into a private mode-`0500` directory, hashed while copied, and removed exactly after every result.
- The host launches the staged executable with one fixed argument, a minimal environment, `/` working directory, concurrent bounded stdout/stderr drains, a timeout, and strict response-field/request/declaration binding.
- Added `hostwright extension check --declaration <absolute-path> --executable <absolute-path> [--output text|json]` with stable `HW-EXT-001` through `HW-EXT-003` errors.
- XCTest compiles and runs a real Swift extension fixture process for the success path and timeout, output overflow, nonzero exit, malformed/duplicate/extra output, binding mismatch, unexpected stderr, digest mismatch, policy blocker, symlink, permission, and exact cleanup paths. Built-CLI integration runs text and JSON handshakes and verifies parent secret-like environment values are not inherited.

Phase 41 proves only local protocol compatibility and exact file/declaration binding for a reviewed executable. The protocol grants no Hostwright capability, but reviewed code retains the invoking account's ambient file, process, and network privileges. This phase does not add generic plugin loading, discovery, installation, persistence, package distribution, capability payloads or invocation, untrusted code execution, sandboxing, ambient-privilege restriction, child-process containment guarantees, RuntimeAdapter APIs, Apple container commands in Hostwright sources, SQLite/state APIs, secret resolution, provider behavior, accelerators, remote control, dependencies, release tags, GitHub Releases, website work, or GUI code.

## Phase 42 Outputs

- Added reusable `HostwrightCLI` command contracts plus the thin `HostwrightCommand` executable entry point without changing the `hostwright` command surface.
- Added `HostwrightControl` and `hostwright-control`, which read exactly one bounded version-1 JSON request from stdin, write exactly one JSON response, and exit.
- Launch arguments require an explicit absolute manifest path and optionally fix explicit state-database and team-profile paths. Requests cannot select or override paths.
- Supported operations are limited to `plan`, `status`, `events`, `recovery`, and `doctor`; each delegates to the existing JSON CLI contract and preserves its exit code and redacted result or error.
- Requests reject unknown or duplicate fields, unsupported versions, unsafe identifiers, invalid filters, and every mutation operation. Configured files must be regular non-symlink files with safe ownership and mode.
- XCTest uses real pipes, files, manifest parsing, team-profile parsing, and SQLite rows. Built-tool integration verifies one-response framing, a real plan request, structured mutation rejection, and no hidden state-database creation.

Phase 42 is runtime-non-mutating, not side-effect-free: `status` with an explicit state database retains existing CLI observation, schema migration, snapshot, and audit writes. The tool does not add apply, cleanup, logs, diagnostics export, benchmark, extension invocation, a persistent daemon, socket, HTTP server, background service, remote control, direct RuntimeAdapter/SQLite access, default paths, dependencies, release tags, GitHub Releases, website work, or GUI code.
