...
# This harness intentionally keeps Gate 5 evidence isolated and append-only.  It is
# not a development test runner: a failed qualification remains inspectable.
readonly schema='hostwright.phase09.gate05.qualification.manifest.v1'
readonly gate=5
...
readonly signing_identifier='dev.hostwright.rbac-qualification'
...
contract() {
  cat <<'EOF'
Phase 09 Gate 5 qualification harness contract v1
Gate 5 — 31.25% — least-privilege RBAC (#199).
Exactly one Gate 5 qualification may be active. Cells 1..6 run strictly serially.
The six cells provide focused U/I/L/M/S/R evidence. Passing evidence is reused only
when source, configuration, toolchain, recorded logs, CMS signature, and checksums
match exactly. Any failure freezes progress, preserves immutable evidence and locks,
and requires a new evidence root after a corrective committed change.
EOF
}
...
validate_root() {
  ...
  if test_mode; then
    ...
    [[ "$canonical_parent" == "$live_parent" ]] || die 'Gate 5 evidence must use the fixed qualification parent.' 66
  fi
  ...
  [[ "${root##*/}" =~ ^phase09-gate05-[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$ ]] || die 'evidence root name is not a Gate 5 lowercase UUID.' 66
...
toolchain_report() { { /usr/bin/sw_vers; xcodebuild -version; swift --version; xcrun --find codesign; security find-identity -p codesigning -v; } 2>&1; }

cell_command() {
  case "$1" in
    1) printf '%s\n' "swift test --filter 'RBACAuthorizationEngineTests|RBACAdministrationServiceTests|RBACRepositoryTests'" ;;
    2) printf '%s\n' "swift test --filter 'PersistentControlAuditIntegrationTests|RBACControlOperationsTests|HostwrightDaemonControlServiceTests'" ;;
    3) printf '%s\n' 'signed hostwright-rbac-qualification --root live-runtime-v1' ;;
    4) printf '%s\n' "swift test --filter 'RBACSchemaV20MigrationTests|ControlIdentityBootstrapTests|StateUpgradeTests'" ;;
    5) printf '%s\n' "swift test --filter 'RBACAuthorizationEngineTests/(testExplicitDenyOverridesAllowsWithDeterministicRuleExplanation|testHorizontalAndVerticalEscalationAttemptsDeny|testMalformedOrConflictingTargetFieldsFailClosedBeforePolicyEvaluation)|RBACAdministrationServiceTests/(testNonOwnerConditionalAndUnprivilegedGrantsAreRejected|testDirectNonOwnerManagementCallsRequireExactPolicyVerbs)|RBACRepositoryTests/(testBootstrapRefusesToMintOwnerWhenMultipleActiveIdentitiesExistWithoutAnOwner|testBindingsRejectDuplicatesAndProtectLastActiveOwner)'" ;;
    6) printf '%s\n' "swift test --filter 'RBACAuthorizationEngineTests/(testDecisionCacheInvalidatesForBindingRoleAndDelegationChangesWithoutRestart|testActiveDelegationWorksThenRevocationAndExpiryStopItImmediately)|RBACRepositoryTests/(testBindingsRejectDuplicatesAndProtectLastActiveOwner|testBindingsAndDelegationsPersistAcrossReopen)|HostwrightDaemonControlServiceTests/testServiceCanRestartAfterOwnedSocketCleanup'; swift build --product hostwrightd; swift build --product hostwright-control; swift build --product hostwright-rbac-qualification; scripts/lint.sh; git diff --check; scripts/check-docs.sh" ;;
    *) die 'unknown Gate 5 cell.' 70 ;;
  esac
}
cell_classes() { case "$1" in 1) echo '["U"]';; 2) echo '["I"]';; 3) echo '["L"]';; 4) echo '["M"]';; 5) echo '["S"]';; 6) echo '["R","I"]';; *) die 'unknown Gate 5 evidence class.' 70;; esac; }
config_digest() { { sha256_file "$0"; sha256_file scripts/lint.sh; sha256_file scripts/check-docs.sh; for cell in 1 2 3 4 5 6; do cell_command "$cell"; cell_classes "$cell"; done; } | sha256_stream; }
collect_digests() {
  ...
  if ! test_mode; then [[ -z "$(git status --porcelain --untracked-files=all -- . ':(exclude)tmp')" ]] || die 'Gate 5 qualification requires an exact clean committed branch.' 73; fi
}
evidence_by_cell_json() { local cell; for cell in 1 2 3 4 5 6; do /usr/bin/jq -n --argjson cell "$cell" --arg command "$(cell_command "$cell")" --argjson evidenceClasses "$(cell_classes "$cell")" '{cell:$cell,command:$command,evidenceClasses:$evidenceClasses}'; done | /usr/bin/jq -s .; }
...
validate_prepared() {
  ...
  [[ "$(/usr/bin/jq -r '.schema' "$root/manifest-v1.json")" == "$schema" && "$(/usr/bin/jq -r '.gate' "$root/manifest-v1.json")" == 5 && "$(/usr/bin/jq -r '.sourceDigest' "$root/manifest-v1.json")" == "$source_digest_value" && "$(/usr/bin/jq -r '.configDigest' "$root/manifest-v1.json")" == "$config_digest_value" && "$(/usr/bin/jq -r '.toolchainDigest' "$root/manifest-v1.json")" == "$toolchain_digest_value" ]] || die 'prepared evidence dependencies changed; preserve this root.' 73
}

record_live_root() {
  local runtime="$1"; [[ "$runtime" == "$root/live-runtime-v1" && -d "$runtime" && ! -L "$runtime" && "$(/bin/realpath "$runtime")" == "$runtime" && "$(stat -f '%u' "$runtime")" == "$(id -u)" && "$(stat -f '%Lp' "$runtime")" == 700 ]] || die 'Gate 5 live runtime root is unsafe.' 70
  printf '%s\ttemporary-root\tgate05-live-runtime\t%s\t%s\t%s\tpath=live-runtime-v1;scope=gate05-live\n' "$(now)" "$runtime" "$(stat -f '%d' "$runtime")" "$(stat -f '%i' "$runtime")" >> "$root/ownership-v1.tsv"; chmod 600 "$root/ownership-v1.tsv"
}
record_live_file() {
  local runtime="$1" path="$2" name device inode; name="${path##*/}"
  [[ "$(dirname "$path")" == "$runtime" && -f "$path" && ! -L "$path" && "$(/bin/realpath "$path")" == "$path" && "$(stat -f '%u' "$path")" == "$(id -u)" ]] || die 'Gate 5 live artifact is not an owned direct regular file.' 70
  case "$name" in
    signed-rbac-tool|state.sqlite|state.sqlite-wal|state.sqlite-shm) ;;
    *)
      [[ "$name" =~ ^\.hostwright-[a-f0-9]{16}-access-v1\.lock(\.writer)?$ ]] \
        || die 'Gate 5 live runtime contains an unowned file.' 70
      ;;
  esac
  device="$(stat -f '%d' "$path")"; inode="$(stat -f '%i' "$path")"
  printf '%s\ttemporary-file\t%s\t%s\t%s\t%s\tname=%s;scope=gate05-live\n' "$(now)" "$name" "$path" "$device" "$inode" "$name" >> "$root/ownership-v1.tsv"; chmod 600 "$root/ownership-v1.tsv"
}
record_live_inventory() {
  ...
  while IFS= read -r -d '' path; do
    ...
    [[ "$recorded" == 1 ]] || { [[ "$recorded" == 0 ]] || die 'Gate 5 live artifact has duplicate ledger records.' 70; record_live_file "$runtime" "$path"; }
    count=$((count + 1))
  done < <(/usr/bin/find "$runtime" -mindepth 1 -maxdepth 1 -type f -print0)
  [[ "$count" -ge 2 && -f "$runtime/signed-rbac-tool" && -f "$runtime/state.sqlite" ]] || die 'Gate 5 live runtime inventory is incomplete.' 70
  [[ -z "$(/usr/bin/find "$runtime" -mindepth 1 -maxdepth 1 ! -type f -print -quit)" ]] || die 'Gate 5 live runtime contains a non-file child.' 70
}
cleanup_live_runtime() {
  local runtime="$1" entry root_device root_inode path device inode
  entry="$(/usr/bin/awk -F $'\t' '$2 == "temporary-root" && $3 == "gate05-live-runtime" {print $4 "\t" $5 "\t" $6}' "$root/ownership-v1.tsv")"; [[ -n "$entry" && "$entry" != *$'\n'* ]] || die 'Gate 5 live root ledger entry is missing.' 70
  root_device="${entry#*$'\t'}"; root_device="${root_device%%$'\t'*}"; root_inode="${entry##*$'\t'}"
  [[ "$entry" == "$runtime"$'\t'* && -d "$runtime" && ! -L "$runtime" && "$(/bin/realpath "$runtime")" == "$runtime" && "$(stat -f '%d' "$runtime")" == "$root_device" && "$(stat -f '%i' "$runtime")" == "$root_inode" ]] || die 'Gate 5 live root identity changed; cleanup is refused.' 70
  while IFS=$'\t' read -r path device inode; do
    [[ "$(dirname "$path")" == "$runtime" && -f "$path" && ! -L "$path" && "$(/bin/realpath "$path")" == "$path" && "$(stat -f '%d' "$path")" == "$device" && "$(stat -f '%i' "$path")" == "$inode" ]] || die 'Gate 5 live artifact identity changed; cleanup is refused.' 70
    /bin/unlink "$path"
  done < <(/usr/bin/awk -F $'\t' '$2 == "temporary-file" {print $4 "\t" $5 "\t" $6}' "$root/ownership-v1.tsv")
  [[ -z "$(/usr/bin/find "$runtime" -mindepth 1 -maxdepth 1 -print -quit)" ]] || die 'Gate 5 live root is not empty after owned-file cleanup.' 70
  /bin/rmdir "$runtime"
}
...
all_reusable() {
  ...
  for cell in 1 2 3 4 5 6; do
    /usr/bin/awk -F $'\t' -v cell="$cell" -v source="$source_digest_value" -v config="$config_digest_value" -v toolchain="$toolchain_digest_value" '$1 == 5 && $2 == cell && $3 == "pass" && $4 == source && $5 == config && $6 == toolchain {found=1} END {exit(found ? 0 : 1)}' "$root/state-v1.tsv" || return 1
    stdout="$root/cell-$(printf '%02d' "$cell").stdout.log"; stderr="$root/cell-$(printf '%02d' "$cell").stderr.log"; [[ -f "$stdout" && -f "$stderr" ]] || return 1
    stdout_sha="$(sha256_file "$stdout")"; stderr_sha="$(sha256_file "$stderr")"; recorded_stdout="$(/usr/bin/awk -F $'\t' -v cell="$cell" '$1 == 5 && $2 == cell && $3 == "pass" {value=$9} END {print value}' "$root/state-v1.tsv")"; recorded_stderr="$(/usr/bin/awk -F $'\t' -v cell="$cell" '$1 == 5 && $2 == cell && $3 == "pass" {value=$10} END {print value}' "$root/state-v1.tsv")"; [[ "$stdout_sha" == "$recorded_stdout" && "$stderr_sha" == "$recorded_stderr" ]] || return 1
  done
}
verify_evidence_digest() {
  local decoded status=0 line_count; [[ -f "$root/evidence-v1.sha256" && ! -L "$root/evidence-v1.sha256" && -f "$root/evidence-v1.cms" && ! -L "$root/evidence-v1.cms" ]] || return 1
  ...
  decoded="$(/usr/bin/mktemp /tmp/hostwright-phase09-gate05-evidence.XXXXXX)" || return 1
...
release_locks_on_success() { if [[ "$run_succeeded" == 1 && "$root_lock_created" == 1 && "$gate_lock_created" == 1 ]]; then /bin/rmdir "$root/active-run-v1"; /bin/rmdir "$parent/.phase09-gate05-active-v1"; root_lock_created=0; gate_lock_created=0; fi; }
...
run() {
  validate_prepared
  if [[ -e "$root/evidence-v1.sha256" || -e "$root/evidence-v1.cms" ]]; then [[ "$(/usr/bin/jq -r '.status' "$root/manifest-v1.json")" == passed ]] && all_reusable && verify_evidence_digest || die 'completed evidence is incomplete or changed; preserve this root and do not rerun.' 73; printf '%s\n' 'Gate 5 evidence is valid and reused; no cells were rerun.'; return; fi
  local gate_lock="$parent/.phase09-gate05-active-v1" cell command started finished status stdout stderr stdout_sha stderr_sha
  [[ ! -e "$root/active-run-v1" && ! -e "$gate_lock" ]] || die 'An active Gate 5 qualification already exists; do not duplicate it.' 75
  ...
  for cell in 1 2 3 4 5 6; do command="$(cell_command "$cell")"; stdout="$root/cell-$(printf '%02d' "$cell").stdout.log"; stderr="$root/cell-$(printf '%02d' "$cell").stderr.log"; [[ ! -e "$stdout" && ! -e "$stderr" ]] || die 'Cell logs already exist; preserve this root and do not rerun.' 73; started="$(now)"; set +e; (set -e; run_cell "$cell") > "$stdout" 2> "$stderr"; status=$?; set -e; chmod 600 "$stdout" "$stderr"; stdout_sha="$(sha256_file "$stdout")"; stderr_sha="$(sha256_file "$stderr")"; finished="$(now)"; if [[ "$status" != 0 ]]; then append_state "$cell" failed "$started" "$finished" "$stdout_sha" "$stderr_sha"; append_failure "$cell" "$status" "$command" "$stdout_sha" "$stderr_sha"; set_manifest_status failed "$finished"; die "Gate 5 cell $cell failed; progress is frozen and locks are preserved." "$status"; fi; append_state "$cell" pass "$started" "$finished" "$stdout_sha" "$stderr_sha"; done
  set_manifest_status passed "$(now)"; /bin/cp "$gate_lock/info-v1.tsv" "$root/gate-active-run-v1-info.tsv"; chmod 600 "$root/gate-active-run-v1-info.tsv"; write_evidence_digest; /bin/unlink "$gate_lock/info-v1.tsv"; run_succeeded=1; release_locks_on_success; printf '%s\n' 'Gate 5 qualification passed.'
}
main() { [[ "$#" -ge 1 ]] || die 'usage: phase09-gate05-qualification.sh <contract|prepare|run>.' 64; case "$1" in contract) [[ "$#" == 1 ]] || die 'contract accepts no arguments.' 64; contract;; prepare) [[ "$#" == 2 && "$2" == 5 ]] || die 'Gate 5 harness accepts only prepare 5.' 64; validate_worktree; validate_root; require_empty_root; collect_digests; prepare; echo 'Gate 5 evidence root prepared.';; run) [[ "$#" == 2 && "$2" == 5 ]] || die 'Gate 5 harness accepts only run 5.' 64; validate_worktree; validate_root; collect_digests; run;; *) die 'unknown Gate 5 qualification command.' 64;; esac; }
main "$@"
