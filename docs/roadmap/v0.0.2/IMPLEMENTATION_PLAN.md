...
| Default paths and local install | Secure Application Support defaults, private permissions, override precedence, truthful status, and journaled legacy-state migration are implemented; installation remains source-only. | Vendor packaging, installed PATH/receipt ownership, upgrade/rollback, repair, and uninstall behavior remain incomplete. | Yes. | P02. | None. |
...
| Manifest v3 | Strict nested request/limit resources, separate scheduling policy, deterministic legacy migration, direct scheduler-admission mapping, and the bounded single-host apply/release lifecycle handoff are implemented in the active boundary. | Aggregate optimization, pressure, accelerator, and Phase 10 capability-promotion evidence remain incomplete. | Yes. | P10 / #207. | v1/v2 remain explicit migration inputs; unknown or unenforceable fields fail closed. |
...
| Observability | Events, bounded logs, local diagnostics, OSLog, durable watches, bounded metrics/SLOs, correlated local traces, privacy-safe support bundles, and retention. | Aggregate leak/soak qualification is incomplete. | Yes. | P08. | No silent telemetry or automatic upload. |
...
| Scheduler | Pure `HostwrightScheduler` contracts, hard filters, deterministic scoring/explanations, durable admission bindings, and bounded single-host Control 2.2 apply/release are implemented for the experimental boundary. | Aggregate G13–G15 optimization, pressure, accelerator, live/hardware/security qualification, and capability promotion remain incomplete. | Yes. | P10 / #219. | There is no advisory or compatibility scheduler; invalid or nondeterministic placement fails the gate. |
...
| 08 | 2026-07-20 | [#194](https://github.com/hostwright/hostwright/issues/194) | Autonomous daemon, rollout/recovery, finalizers/GC, OSLog, watches, metrics, traces, support bundles. | P02–P07 | Kill-at-every-checkpoint suite, 10,000 cycles, cumulative checkpointed 72-hour single-host soak. |
...
The contract below is a locked qualification target, not a claim that G13–G15 has passed. The current Phase 10 implementation is experimental: the pure scheduler, manifest bridge, durable state contracts, and bounded single-host Control 2.2 admission/apply/release path are independently testable, and the current-source G3-G8 scheduler qualification is sealed with zero safety mismatches. The sealed evidence covers 1,000,000 generated cases, 10,000 exact-oracle cases with 382 retained intentional optimization-gap fixtures, and a 0.917259875-second p95 on Mac16,8; it does not close G13-G15 or promote `scheduler.optimization`. Aggregate live, hardware, security, optimization, pressure, and accelerator evidence remain pending under their owning workstreams, and `accelerators.host-native` remains unavailable.
...
| Lifecycle reliability | 10,000 lifecycle cycles and a cumulative checkpointed 72-hour single-host soak without duplicate resources, unmanaged mutation, or monotonic leaks; gaps and incomplete intervals do not count. |
