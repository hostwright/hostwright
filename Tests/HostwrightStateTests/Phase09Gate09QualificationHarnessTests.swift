...
final class Phase09Gate09QualificationHarnessTests: XCTestCase {
  ...
  func testAutonomousConvergenceUsesPendingClaimAndMetricsExportMutation() throws {
    ...
    let metricsExport = try XCTUnwrap(live.range(of: "\"$cli\" metrics export --state-db \"$state\""))
    ...
    XCTAssertLessThan(releaseFence.lowerBound, quiesce.lowerBound)
    XCTAssertLessThan(releaseFence.lowerBound, metricsExport.lowerBound)
