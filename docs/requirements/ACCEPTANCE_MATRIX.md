...
| HW-RECON-005, HW-STATE-003, HW-OBS-001 | Managed restart writes append-only operation records, restart recovery records, restart policy state, and redacted events for success and failure. | Automated | CLI and state XCTest cases for success, failure hints, backoff, and redaction. |
