...
Unattended recovery reads the latest persisted lifecycle intent in insertion order. A successful `up`, `start`, `restart`, or `update` can supply continued run intent for its exact resource generation; a later `down`, `stop`, or `rm` suppresses it. Failed or interrupted intent requires ordinary saga recovery. Historical reservations supply identity and original admission evidence, but released capacity must be admitted again. Before recovery effects, the daemon checks the declaring subject's current identity, credential expiry, project authorization, admission policy, and exact resource scope. Mixed-owner selections remain in a safe hold. The raw configuration-file digest remains distinct from the lifecycle digest that also binds image policy material.
...
The pure scheduler, manifest bridge, policy checks, migration/repository behavior, authenticated Control 2.2 admission/release path, and owned lifecycle handoff form the bounded single-host admission slice. Earlier G3-G8 receipts and 382 intentional optimization-gap fixtures remain historical development evidence. They do not qualify changes to the local lifecycle path or replace the reduced release's live runtime, recovery, pressure, and distribution gates. Optimization and accelerator work is explicitly deferred under ADR 0015. `scheduler.optimization` and `accelerators.host-native` remain unavailable.
