...
| Daemon | Foreground and exact current-user LaunchAgent modes use the same level-triggered lifecycle saga. One-user LaunchAgent lifecycle, secure config reload, unattended reconciliation, durable restart budgets, bounded backoff, stable reset, and exact manual hold release are implemented; later Phase 08 policy/recovery/observability and soak gates remain incomplete. | Repeat the complete lifecycle on the supported macOS matrix and pass later unattended-policy/soak gates before GA. |
...
| State | SQLite schema v17 for standalone/node-local state, including immutable image digest locks, verified OCI referrer graph evidence, image-trust audit, exact image-SBOM records, signed vulnerability-report decisions/exceptions, exact signed build-provenance evidence, provider-scoped content accounting, exact storage volume/snapshot/backup/orphan/quota records, UUID/fence-bound network authority, and durable restart budgets/history. | Qualified backup/restore/repair and etcd 3.7.x authority for multi-Mac clusters. |
