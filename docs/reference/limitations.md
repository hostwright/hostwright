...
Phase 10 scheduler admission remains an experimental bounded single-host capability. `HostwrightScheduler` produces pure deterministic decisions, `HostwrightState` owns durable capacity and reservation authority, and the authenticated Control 2.2 lifecycle bridge validates requests, limits, policy, runtime enforceability, owned cleanup, and authoritative absence. Current-source G3-G8 qualification is sealed with zero safety mismatches and 382 retained intentional optimization-gap fixtures. Remaining G13-G15 aggregate optimization and accelerator evidence, remote placement, and external scheduler compatibility remain unavailable.
