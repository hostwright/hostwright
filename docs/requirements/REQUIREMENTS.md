...
| HW-COMPAT-011 | Benchmark reports must record exact environment, disposable-resource policy, measured and unmeasured dimensions, and limits before any performance claim. | Phase 36 maintainer scope; Document 2; Final Production Arsenal | Implemented as local dry-run and fixture-backed report contract only | `Sources/HostwrightHealth/BenchmarkLabModels.swift`, `docs/architecture/benchmark-lab.md` | Health XCTest dry-run and fixture parser cases; core docs guard | 36 |
| HW-COMPAT-012 | CI benchmark work must not require live Apple container, image pulls, runtime mutation, cloud telemetry, or hosted performance monitoring. | Phase 36 maintainer scope | Implemented by hosted CI build/test/lint/naming scan plus fixture-only benchmark tests | `.github/workflows/ci.yml`, `docs/architecture/benchmark-lab.md`, `docs/reference/limitations.md` | Hosted CI check, health XCTest cases, boundary scans | 36 |
...
| ID | Requirement | Source document | Current status | Implementation file if any | Test coverage if any | Release phase |
| --- | --- | --- | --- | --- | --- | --- |
| HW-TEAM-001 | Team policy profiles must be local, explicit opt-in, versioned, auditable, and preserve required Hostwright safety gates. | Phase 34 maintainer scope | Implemented as local policy model and evaluator only | `Sources/HostwrightPolicy/TeamWorkflowPolicy.swift`, `docs/reference/team-workflow.md` | Policy XCTest cases and core docs guard | 34 |
| HW-TEAM-002 | Team policy overrides that weaken required gates require local approval records, and hard safety-gate bypasses remain forbidden even with approval. | Phase 34 maintainer scope | Implemented in `TeamWorkflowPolicyEvaluator` | `Sources/HostwrightPolicy/TeamWorkflowPolicy.swift` | Policy XCTest cases | 34 |
| HW-TEAM-003 | Team workflow audit records must use explicit local state paths and redacted append-only event records. | Phase 34 maintainer scope | Implemented through existing event ledger records | `Sources/HostwrightState/StateRepositories.swift`, `docs/reference/team-workflow.md` | State XCTest audit event persistence test | 34 |
...
| HW-REL-005 | Binary or installer publication requires a clean `v*` tag build, signing, notarization, checksums, SBOM, provenance, install/uninstall proof, upgrade/downgrade notes, rollback instructions, and package-channel approval. | Phase 35 maintainer scope; Final Production Arsenal; Document 2 | Implemented as fail-closed distribution readiness gate only; no artifacts are produced | `docs/release/distribution-readiness.md`, `docs/release/RELEASE_PROCESS.md`, `docs/reference/install.md`, `docs/reference/security-safety.md` | Core release-doc XCTest case | 35 |
| HW-REL-006 | Current docs must not claim binary downloads, installers, signing, notarization, SBOM, provenance, install scripts, Homebrew, or package-channel support until evidence exists. | Phase 35 maintainer scope; Final Production Arsenal | Implemented by docs guard and limitations | `docs/release/distribution-readiness.md`, `docs/reference/limitations.md`, `docs/reference/install.md` | Core release-doc XCTest case | 35 |
